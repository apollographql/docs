...
When you [create a cloud supergraph](../getting-started/#2-create-your-first-supergraph) with Apollo Studio, GraphOS automatically provisions and manages a **router** that sits in front of your GraphQL API:
...
Your router is provisioned _automatically_ whenever you create a cloud supergraph in Studio, or whenever you create a new [variant](../org/graphs/#variants) for an _existing_ cloud supergraph. Each variant has its own distinct router.
...
During the preview phase, infrastructure for cloud routing is in Chicago, USA. Our infrastructure has interconnects with the major public cloud providers, so latency is minimal.

For example, latency between Chicago and AWS in Virginia is approximately 10ms. We will likely be adding available regions in the coming months, so please [reach out](https://docs.google.com/forms/d/e/1FAIpQLSeLfIHulp-45VYzZspkOslFMzd1pPnMtVgIj_pVyJbox5DCKw/viewform) if you have specific requirements.
...
Our goal with cloud routing is to deliver global, industry-leading graph performance. During the preview phase, routing infrastructure is hosted in one region, with plans to expand globally and add caching support in 2023.
