...
When you're done adding tag names, click **Generate Preview** to check the contract schema that your contract will generate. If the preview generates successfully, you can then click **Review** to continue.
...
If you use both contracts and [schema checks](./schema-checks/) in Apollo Studio, you can (and should) configure **contract checks** for any variant that acts as a source variant for contracts. This helps your team identify when proposed changes to a source variant will negatively affect one or more downstream contracts.
...
You're done! From now on, whenever schema checks run for your source variant, those checks include contract checks for the selected contract variants. Those checks are shown in Studio alongside other check types under **Checks > This Variant > Recent Checks**:
...
Click the **Downstream** item in the Tasks column to view the details of the contract check's execution.
