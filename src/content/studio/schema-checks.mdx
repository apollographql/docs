...
2. Make sure your supergraph is reporting metrics to GraphOS. GraphOS uses these metrics to help determine when a type or field can be modified or removed safely.
    - If you have a [cloud supergraph](./org/graphs/#cloud-supergraphs), your router reports metrics automatically.
...
> If you have a monolith [classic graph](./org/graphs/#classic-graphs) (not recommended), use [`rover graph`](/rover/commands/graphs/#checking-schema-changes) commands instead of `rover subgraph` commands. 
...
1. GraphOS generates a diff between your local schema and the registered schema for the variant you're checking against.
