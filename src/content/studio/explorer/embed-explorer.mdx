...
5. You have three options for how you want to embed the Explorer. You can use the `@apollo/explorer` npm package, or hosted built files to embed. The `@apollo/explorer` npm package exports a React component that when rendered, embeds the Explorer, or a Javascript class that you can call to embed. Once you have decided which version to use, click **Copy** to copy the snippet, then paste it into your code.
...
You can also open the Embed Explorer dialog from two other places in Studio, both of which configure your embedded Explorer to include all of the following automatically on load:
...
Open the share dropdown in Explorer, and select "Share as embed".
...
If you are embedding a [public variant](../org/graphs/#public-variants), anyone who has access to the embed will be able to view your schema and make queries against your variant. If you are embedding a private variant, the embedded Explorer will ask you to first authenticate yourself by logging in to Studio, and then you will be asked to authorize your Studio account to be used on this webpage.

<img
  class="screenshot"
  src="../img/explorer-authorize.jpg"
  alt="Explorer authorization screen"
  width="300"
/>

When you accept, you will be able to see the Explorer for this variant, including the schema of the variant and the Operation Collections on this variant.
