...
3. Click **Copy** to copy the snippet, then paste it into your code.
...
This will be the initial endpoint introspected when the Sandbox renders on page load. 
The schema at this endpoint will be used to populate the Sandbox pages. Defaults to `http://localhost:4000`.

In general, we recommend you **only pass development endpoints to Sandbox**.
...
By default, the embedded Sandbox uses the `fetch` API to send requests to the endpoint url at the top of the page from your webpage. You can instead pass a _custom_ fetcher that's used to run all operations.
...
You can pass `includeCookies: true` if you want `{ credentials: 'include' }` to be passed as an option whenever requests are make from the Explorer. If you pass a `handleRequest`, this option will be ignored.
