...
> Note that supergraphs do _not_ currently support subscription operations. You can only execute subscriptions in the Explorer with a monolith [classic graph](../org/graphs/#classic-graphs).
...
If you've enabled [public access to a variant of your graph](../graph-access-permissions/#public-variants), you can embed the Explorer in a webpage that you can then provide to your graph's consumers. This enables those consumers to test out operations from your own website.
...
If you're using Apollo Server 2.18+, you can see traces from your responses inlined in the Explorer by adding the `ApolloServerPluginInlineTrace` plugin to your server's configuration:
