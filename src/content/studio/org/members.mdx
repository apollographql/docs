...
| **Contributor**            | Can push new schemas to graphs, but cannot configure graph settings or integrations. Intended for back-end developers who are authorized to make changes to graphs. _Cannot_ push schema changes to [protected variants](../studio-graph-features#protected-variants-enterprise-only).                                                |                        Enterprise only                         |
...
If a graph is made [hidden](../studio-graph-features/#internal-graph-visibility-enterprise-only), then only users with explicit overrides (as well as **Org Admin**s) can see the graph.
...
| Create new variants                                                |     ✓     |      ✓      | Non-[protected](../studio-graph-features#protected-variants-enterprise-only) variants only |                       |          |
| Push schemas to a graph                                            |     ✓     |      ✓      | Non-[protected](../studio-graph-features#protected-variants-enterprise-only) variants only |                       |          |
| Manage Explorer settings (URL, etc)                                |     ✓     |      ✓      | Non-[protected](../studio-graph-features#protected-variants-enterprise-only) variants only |                       |          |
