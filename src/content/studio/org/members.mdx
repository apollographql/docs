...
| **Contributor**            | Can push new schemas to graphs, but cannot configure graph settings or integrations. Intended for back-end developers who are authorized to make changes to graphs. _Cannot_ push schema changes to [protected variants](../graph-access-permissions#protected-variants-enterprise-only).                                                |                        Enterprise only                         |
...
| **Billing Manager** (free) | <p>Has management access to organization-level configuration and billing. Can also remove members (but not invite them). No access to graphs. [Learn more](#billing-managers)</p><p>This role is available to all paid plan types. **Billing Manager** members do not count as billed users.</p>                       |                Enterprise, Serverless, and Team                |
...
If a graph is made [hidden](../graph-access-permissions/#internal-graph-visibility-enterprise-only), then only users with explicit overrides (as well as **Org Admin**s) can see the graph.
...
| Create and modify [contracts](../contracts/) and contract variants |     ✓     |      ✓      |                                                                             |                       |          |
| View existing contracts                                            |     ✓     |      ✓      |                                      ✓                                      |                       |          |
| Create new variants                                                |     ✓     |      ✓      | Non-[protected](../graph-access-permissions#protected-variants-enterprise-only) variants only |                       |          |
| Push schemas to a graph                                            |     ✓     |      ✓      | Non-[protected](../graph-access-permissions#protected-variants-enterprise-only) variants only |                       |          |
| Manage Explorer settings (URL, etc)                                |     ✓     |      ✓      | Non-[protected](../graph-access-permissions#protected-variants-enterprise-only) variants only |                       |          |
| Create [Deployed Graphs](./graphs/)                                |     ✓     |      ✓      |                                      ✓                                      |                       |          |
