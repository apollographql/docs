...
Operations defined within client applications can be extracted and uploaded to Apollo Studio using the [Apollo CLI](https://www.apollographql.com/docs/devtools/cli/). Apollo Server then fetches a manifest of these operations from Apollo Studio and forbids the execution of any operations that were not in that manifest.
...
- An Apollo Studio API key.
  - To obtain an API key, visit [Apollo Studio](https://studio.apollographql.com) and create a service. You will need a Studio account with a Team or Enterprise subscription to be able to upload to the operation registry.
...
6. [Start Apollo Server with Apollo Studio enabled](#6-start-apollo-server-with-apollo-studio-enabled)
...
> **Note:** If this server's schema has already been [registered](./schema/schema-reporting/), you can skip this step.
