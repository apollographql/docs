...
A **supergraph** (also known as a [federated graph](https://apollographql.com/docs/federation) is a graph that's composed of one or more **subgraphs**:
...
Each [variant](./org/graphs/#variants) of a Studio supergraph has its own subgraph schemas, supergraph schema, change history, and metrics.
...
After you create a [cloud supergraph](./org/graphs/#cloud-supergraphs) (or a new variant of one), you can view a list of its associated subgraphs from the variant's Subgraphs page:
...
- **Deleting a subgraph can break existing clients!** This action is equivalent to removing _all_ of the subgraph's types and fields from your supergraph (unless they're also defined in _another_ subgraph). If a client sends an operation that includes any of those types and fields, the router will _reject_ that operation.
    - To help you remove schema definitions safely, learn about [schema checks](./schema-checks/).
...
- If your subgraph doesn't already have a `schema` object, you need to add one to use the `@contact` directive. The `schema` object can't be empty, so you need to include at least one field in it (most commonly `query: Query`). If you do provide the `Query` type like this, it needs to include at least one field in your subgraph's schema. If your subgraph's `Query` type is empty, you need to add a dummy field for the SDL to be valid:
