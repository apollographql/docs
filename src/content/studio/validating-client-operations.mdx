...
You can confirm that all of the GraphQL operations defined in your client application are valid against the types, fields, and arguments in your graph's [published schema](./schema/cli-registration). To do so, you use the `apollo client:check` command of the [Apollo CLI](https://www.apollographql.com/docs/devtools/cli/):
...
If you represent each of your server's environments with a separate [graph variant](./org/graphs/#variants) (recommended), you can check your operations against a particular variant's schema like so:
