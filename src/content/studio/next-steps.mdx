...
After you [create your cloud supergraph](./getting-started/) in Apollo Studio, read below to learn about some of the next steps you should take to get the most out of it and the entire GraphOS platform.
...
See [this article](./local-development/).
...
After learning about this publishing process, you should [integrate it into your CI workflow](#set-up-continuous-delivery).
...
1. To check whether your current GraphQL server library supports the subgraph specification, see [this article](/federation/building-supergraphs/supported-subgraphs).
    - If your library _does_ support the spec, consult its documentation to learn how to enable its federation support. For Apollo Server, see [this article](/federation/building-supergraphs/subgraphs-apollo-server).
...
1. Verify that your proposed changes won't break your supergraph _or_ any of your existing clients.
    - To do this, you set up [schema checks](./schema-checks/) for your supergraph.
...
GraphOS can send you various types of notifications via a few different channels (including Slack and Pagerduty). These notifications can include a daily metrics report, schema change updates, and experimental performance alerts. [Set up notifications.](./notification-setup/)
