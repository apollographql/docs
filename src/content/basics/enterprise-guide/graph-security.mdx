...
As a best practice for federated graphs, Apollo recommends that subgraph services should only be queried by the gateway or Router directly. The [Apollo Federation specification](https://www.apollographql.com/docs/federation/federation-spec/) outlines that subgraph schemas will have `_entities` and `_service` root fields on the `Query` type to assist with composition and query planning, which exposes the subgraph to additional concerns.
