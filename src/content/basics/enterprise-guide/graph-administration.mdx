...
Apollo Studio can serve as this centralized registry, and subsequently, unlock all of the capabilities of [managed federation](/federation/managed-federation/overview/). Managed federation will create a new gateway configuration every time a schema is pushed to the registry by one of the subgraphs using the `rover subgraph publish` command from the [Rover CLI](/rover/), but only does so when those changes can be composed into the existing graph without breaking changes for other subgraphs and clients. Additionally, managed federation allows teams to create different _variants_ of the graph that correspond to the different environments where the graph runs (such as staging and production). Each variant has its own GraphQL schema, which means schemas can differ between environments.
