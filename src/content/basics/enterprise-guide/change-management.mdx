...
Changing the behavior of a resolver (such as calling a new data source, changing the computation of a value, or optimizing execution by adding [Dataloaders](https://www.apollographql.com/docs/apollo-server/data/fetching-rest#using-with-dataloader)) might be performed in a single release for low-risk changes.
