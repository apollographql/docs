...
<SummitCallout topic="GraphOS features">

Explore GraphOS features in the GraphQL Summit Workshop: [Scaling your graph: Drive efficiency and collaboration with GraphOS](https://reg.summit.graphql.com/flow/apollo/summit24/AttendeePortal/page/catalog/session/1712944865354001K23Q).
...
The diagram above shows a basic supergraph with a GraphOS Router, two subgraphs, and two corresponding data sources.
...
- [The GraphOS Router](/router/) (strongly recommended)
    - The GraphOS Router is powered by the Apollo Router Core—a high-performance binary written in Rust that adds [minimal latency to request processing](https://www.apollographql.com/blog/announcement/backend/apollo-router-our-graphql-federation-runtime-in-rust/), even with extremely high traffic.
    - Because the Apollo Router Core is released as a precompiled binary, you don't need to write any boilerplate code around it to deploy it to your backend. All it needs is a configuration file for any behavior you want to customize.
    - Cloud supergraph routers use the GraphOS Router under the hood.
