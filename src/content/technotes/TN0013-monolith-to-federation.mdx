...
1. Deploy an instance of the [Apollo Router](/router/) to your environment.
  ...
  />
2. Set up [header propagation](/router/configuration/header-propagation/) so that the monolith receives any necessary headers from the router.
3. Set up internal routing rules to redirect client requests to the Apollo Router instead of your monolith.
4. Enable usage metrics reporting to GraphOS Studio.
5. Add [subgraph checks](/graphos/delivery/schema-checks/) to your monolith's CI pipeline.
