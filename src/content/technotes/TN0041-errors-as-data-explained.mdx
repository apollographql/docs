...
description: Union types in GraphQL are an excellent way to represent multiple types of data in a single field. This can be extremely useful when dealing with different types of responses, such as success and error cases.
...
```graphql
union CheckoutResponse = Order | InsufficientStockError | InvalidPaymentMethodError
