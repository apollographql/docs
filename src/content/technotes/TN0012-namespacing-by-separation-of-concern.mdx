...
Instead it is recommended that any GraphQL mutations by specified at the root level so they are executed serially, or that any nested fields are combined so only one action is actually performed.

At this time, there is currently not an official name spacing tool that nested mutations aims to solve, but there has been a [proposal for GraphQL Namespaces](https://github.com/graphql/graphql-spec/issues/163) open for some time where we encourage community members to share their requirements for any spec updates.
