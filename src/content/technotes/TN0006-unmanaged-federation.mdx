...
As described in the [Build status webhook section](#build-status-webhook), make sure to use [run schema checks in your CI/CD pipeline](/rover/commands/subgraphs/#checking-subgraph-schema-changes) or during build time to verify that any changes won't break existing clients. 
...
In the code above, the `ApolloGateway` is passed a `supergaphSdl` property. This property can be a string (for a supergraph schema that _never_ changes), or it can be a function that enables updating the schema during runtime as shown.  
