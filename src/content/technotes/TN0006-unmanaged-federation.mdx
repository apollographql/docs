...
If you are using `@apollo/gateway` locally and have access to query subgraphs ([not through public internet](https://www.apollographql.com/docs/enterprise-guide/graph-security/#only-allow-the-gateway-to-query-subgraphs-directly)), you can use [IntrospectAndCompose](https://www.apollographql.com/docs/federation/api/apollo-gateway/#class-introspectandcompose).
...
This will use the [`_service` field](https://www.apollographql.com/docs/federation/federation-spec#query_service) to fetch the schema information directly from a running subgraph instead of fetching from Studio. If you are using [Managed Federation](https://www.apollographql.com/docs/federation/managed-federation/overview/) then this field is not needed.
