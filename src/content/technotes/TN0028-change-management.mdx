...
subtitle: Ensure smooth releases and minimize client impact
description: Use tools like GraphOS Studio, Rover, and contracts to ensure smooth releases and minimize client impact.
...
- [Backward incompatible subgraph schema changes](#backward-incompatible-subgraph-schema-changes)
...
- We run build checks for a proposed change against the `current` variant (to catch build errors quickly) as well as our production variants (to run operations checks against live traffic).
