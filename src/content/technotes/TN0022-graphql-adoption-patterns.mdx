...
Federation uses a declarative programming model that enables each subgraph to implement _only_ the part of your graph that for which it’s responsible. With this approach, your company can represent an enterprise-scale graph as a collection of separately maintained GraphQL services.
...
[Apollo Router](/router/) is Apollo's next-generation graph runtime that enables federated supergraphs with fast and consistent performance.
