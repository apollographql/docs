...
The biggest goal of any API strategy should be to enable consumers. In practice, this means having as low of friction as possible for consumers to get started consuming your API. The more a consumer can self-serve, the better.
...
- ❌ Does not Magnify Existing Investments
- ❌ While an API Gateway could allow for some governance of BFFs, it still doesn't not allow much governance of the data retrieval.
...
> ✅ GraphQL enables [rapid self service](#offer-rapid-self-service) for our presentation clients by replacing one-off handwritten BFF apis with declarative queries.
...
### Insulating Contract
> ✅ GraphQL provides an [insulating contract](#create-an-insulating-contract) between our Presentation and Application layers that is flexible enough to manage the constant change while stable enough to prevent breaking changes.
...
### Magnify Investments
> ✅ GraphQL goes beyond just preserving the value of our existing API's, to actually increasing and [magnifying the value](#magnify-existing-investments) through federation and its ability to connect and compose independent APIs together.
...
### Governance
> ✅ GraphQL brings a declarative layer and a control plane to the middle, enabling us to [apply declarative governance](#govern-the-middle-layer) in this crucial layer.
