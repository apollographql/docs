...
To improve the logical organization of our graph's capabilities, we can define **namespaces** for our top-level operation fields. These are object types that in turn define query and mutation fields that are all related to a particular concern.
...
Finally, we can add our namespace types as the return values for top-level fields of the `Mutation` type:
...
Unlike all other fields in a GraphQL operation, the top-level fields of the `Mutation` type must be resolved _serially_ instead of in parallel. This can help prevent two mutation fields from interacting with the same data simultaneously, which might cause a race condition.
