---
title: Optimizing Apollo Router container builds with Kaniko
id: TN0030
tags: [router]
---
...
Building the Apollo Router including custom plugins from source is a memory-expensive process, as such it’s not suited for all environments.
...
The router will also download a large collection of dependencies during the build process, as such we can use the [Kaniko](https://github.com/GoogleContainerTools/kaniko) project which will cache intermediate build layers.
...
This is all thats needed to enable caching of the intermediate build steps which will be pushed to the destination container repo as soon as each layer is built.
So even failed builds will contribute to increased build speeds as successful layers are cached. We also change the default `options.machineType` value to `E2_HIGHCPU_8`, this is because the default worker pool doesn’t have the cpu or memory resources necessary to compile the router.
