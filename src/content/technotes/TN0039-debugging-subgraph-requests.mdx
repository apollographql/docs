---
title: Debugging subgraph requests from Apollo Router or @apollo/gateway
description: As your graph grows, you may need to debug a problematic query for one reason or another. The Apollo Router and @apollo/gateway both serve as an entry point into your federated graph and offer ways to debug requests.
published: 2023-04-13
id: TN0039
tags: [federation, gateway, router, subgraphs]
---

As your graph grows, you may need to debug a problematic query for one reason or another. The [Apollo Router](/router) and [`@apollo/gateway`](/apollo-server/using-federation/api/apollo-gateway/) both serve as an entry point into your [federated graph](/federation) and offer ways to debug requests.
...
If, instead, you want to debug your subgraph HTTP requests in an Apollo Router instance, you can use [Rhai scripts](/router/customizations/rhai) to log the necessary information out. An example Rhai script is shown below.
...
To enable query plans, you must run the Apollo Router with the `--dev` flag and leverage [Apollo Sandbox](https://studio.apollographql.com/sandbox) to [display your query plans](/graphos/explorer/additional-features/#query-plans-for-supergraphs).
