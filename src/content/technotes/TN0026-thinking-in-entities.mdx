...
Referencing entities is a key feature of federation, but it's only half of the story. While an entity will be owned by a single subgraph, other subgraphs may wish to add additional fields to the entity's type to provide a more holistic representation of the entity in the graph. Doing so is a simple as adding the additional field to the extended type in a non-originating subgraph. For example, a reviews subgraph's schema may add a `reviews` field to the extended `Product` type that was originally defined in the products subgraph:
...
Entities aren't just useful to connect data across subgraphs; entities can be used to enable [@defer support within your supergraph by using entity-based defer within the Apollo Router](/router/executing-operations/defer-support). By creating entities within your graph, you can then have the router handle the asynchronous fetching of data to enable full defer support which can have material user experience improvements via an improved time to first byte, among other metrics.
