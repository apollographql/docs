...
<EnterpriseFeature text={`This is an [Enterprise feature](/router/enterprise-features) of the Apollo Router. It requires an organization with a [GraphOS Enterprise plan](https://www.apollographql.com/pricing/).`} />
...
In addition to the the approaches above, you can use [authorization directives](/router/configuration/authorization/) to enforce authorization at the router layer. 
...
Once the request's "claims" are made available via the JWT and/or a coprocessor, they can be used as scopes to enforce authorization policies.
...
Cons
- Updates will need to be made to each subgraph to opt into this authorization model
- If a coprocesser is required, initial lift of implementing a coprocessor is non-trivial
...
3. Use authorization directives to enforce authorization at the router layer
