...
If the `Query.topReviews` field returns 10 reviews, then the executor resolves `Review.product` field 10 times. If the `Reviews.product` field makes a database or REST query for a single `Product`, then we'll see 10 unique calls to the data source. This is suboptimal for two reasons:

1. It's more efficient to fetch the 10 products in a single query (e.g. `SELECT * FROM products WHERE id IN (<product ids>)`).
2. If any reviews refer to the _same_ product, then we're wasting resources fetching something we already have.
...
1. First, we `Fetch` the list of `Review`s from the Reviews subgraph using the root field `Query.topProducts`. We also ask for the `id`s of each associated review.
2. Next, we extract the `Product` _entity references_ and `Fetch` them in a **batch** to the Product subgraph's `Query._entities` root field.
...
In most subgraph implementations, including [`@apollo/subgraph`](/federation/api/apollo-subgraph), we don't write the `Query._entities` resolver directly. Instead, we have a [reference resolver API](/federation/entities#2-define-a-reference-resolver) for resolving an individual entity reference.
...
The motivation for this API relates to a subtle, critical aspect of the [subgraph specification](/federation/subgraph-spec): **the order of entity references must match the order of returned entities**. If the entities resolved by the `Query._entities` field come back in the wrong order, the results are merged with the wrong entities and we'll have incorrect results. To avoid this footgun, most subgraph libraries handle entity order for you.
