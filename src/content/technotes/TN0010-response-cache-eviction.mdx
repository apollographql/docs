...
This whole solutions works by defining a custom response cache key by a pattern that can later be searched on in the cache. What this key should be comprised of and how it should be structured depends on the use-case and what search patterns our cache implementation supports. 

### A Note on Cache Key Uniqueness

Keep in mind that each key links to a full response object, so if your key is too generic, you risk potentially returning the wrong data for queries. I.E, generating a cache key based solely on the operation name would yield the same responses for all operations with the same name, even if the entire query is different. 

Put simply, make sure your keys are unique enough for each incoming operation that returns different data. 

### Defining a Custom Cache Key

Version `3.7.0` of the `apollo-server-plugin-response-cache` introduced the `generateCacheKey` configuration method. Whatever is returned from this function will be used as the cache key to store the current query response
...
There are two main strategies for evicting response cache entries: Manually evicting from a shell prompt, or in response to some event, like a mutation. Actually removing entries from the cache once a custom cache key is being used will depend on your caching backend. Different caching tools offer different ways to list and remove keys. We’ll explore both options using Redis. 
