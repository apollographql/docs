...
Federation will apply the same orchestration capabilities to mutation fields (and their selection sets), but managing state changes across distributes systems typically involves additional requirements. Federation does not provide common functionality such as compensating transactions or data propagation between sibling fields.

You will need to implement your own orchestration logic within a single field resolver to implement these requirements. This is due to the design of GraphQL itself.
...
Even if all three of these mutations were implemented in the same service, it's difficult to implement the appropriate transactional semantics when run within the GraphQL execution engine. We should instead implement this operation as a single mutation resolver so that we can handle failures and rollbacks in one function.
...
There are no universally correct answers to these questions. It will depend on your organization, your product requirements, and the capabilities of your system.

A potential strategy is to create one or more subgraphs specifically for orchestrating mutations. They can communicate with data services using REST or RPC. Experience or product teams are often the most appropriate owners of these subgraphs.

It's tempting to have the orchestrating subgraph resolvers call back to Apollo Router to execute domain-specific mutations. This approach is feasible but requires careful attention to a number of details:
