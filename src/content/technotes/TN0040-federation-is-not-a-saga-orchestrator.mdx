...
While you can expose mutations in your supergraph, and the query planner will call various systems to resolve mutation fields, Federation does not provide common distributed systems functionality such as compensating transactions, retries, or data propagation.

You will need to implement your own orchestration logic within single field resolvers to implement these requirements. This is in fact due to the design of GraphQL itself.
...
- We don't have a way to roll back the changes made by `createAccount` if `validateAccount` or `setAccountStatus` fail.
...
Even if all three of these mutations were implemented in the same service, it's difficult to implement transactionality when run within the GraphQL execution engine. We should implement this operation as a single mutation resolver so that we can handle failures and rollbacks in one function.
...
- What team or domain owns this mutation field?
- In which subgraph service should we implement the resolver?
- How does the resolver communicate with the data services?

There are no right answers to these questions. The answer depends on your organization and the requirements of your system.

A potential strategy might be to create one or more subgraphs specifically for orchestrating mutations. They can communicate with data services using REST or RPC. Experience or product teams are often the most appropriate owners of these subgraphs.

It's tempting to have the orchestrating subgraph resolvers call back to Apollo Router to execute low-level mutations. This approach is feasible but requires careful attention to a number of details:

- The calls to low-level mutations no longer originate in client applications, so you must propagate client identity and other request metadata.
- GraphOS will view these calls as separate operations.
- If using cloud-native telemetry, you must ensure that Apollo Router receives the trace context to correlate spans.
- Be wary of circular dependencies. Consider using [Contracts](/graphos/delivery/contracts/) to create variants specific to orchestration and eliminate the potential for loops in an operation.
