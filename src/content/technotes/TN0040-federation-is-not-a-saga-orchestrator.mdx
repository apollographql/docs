...
- All calls should go through the router, not directly to subgraphs, because it's responsible for tracking GraphQL usage (and [subgraphs should never accept traffic directly](/technotes/TN0021-graph-security/#only-allow-the-router-to-query-subgraphs-directly).)
