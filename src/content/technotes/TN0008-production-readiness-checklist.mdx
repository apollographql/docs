...
## Apollo Router
- Ensure that you've correctly configured [managed federation](/federation/managed-federation/setup/) and [GraphOS schema usage reporting](/router/configuration/apollo-telemetry).
- For security, [disable introspection](/router/configuration/overview/#introspection) for all production routers (by default Router disables introspection, but makes sure you are not using `--dev` mode).
  - You can continue to view and fetch your GraphQL schemas from Apollo Studio and run operations from [Studio Explorer](/graphos/explorer).
- Configure the [Router traffic shapping](/router/configuration/traffic-shaping) features:
  - Set request and subgraph level timeouts and rate limits
  - Deduplicate subgraph requests
  - Communicate with subgraphs using APQ
- Enable [operation limits](/router/configuration/operation-limits) to block large and malicous requests
- Configure additional [tracing, metrics, and logging](router/configuration/tracing) through OpenTelemetry or Prometheous

## Subgraphs/Servers
- For security, [disable introspection](/apollo-server/api/apollo-server/#introspection) for all production GraphQL subgraphs.
