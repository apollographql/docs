...
- Ensure that you've created multiple [variants](/graphos/org/graphs#variants) to represent the different environments where your supergraph runs (production, staging, and so on).
  - Optionally, [protect your production variant](/graphos/studio-graph-features#protected-variants-enterprise-only) to avoid accidental changes while working in Apollo Studio.
