...
- Ensure that you've created multiple [variants](/studio/org/graphs#variants) to represent the different environments where your supergraph runs (production, staging, and so on).
  - Optionally, [protect your production variant](/studio/graph-access-permissions#protected-variants-enterprise-only) to avoid accidental changes while working in Apollo Studio.
- Ensure that your [clients identify themselves](/studio/metrics/client-awareness/) by name and version.
...
- If your [subgraph servers are listed as compatible with `ftv1`](/federation/supported-subgraphs/), ensure that you've enabled [federated traces](/federation/metrics/), and that you can view operation metrics as expected in Apollo Studio.
