...
- Ensure that you've created multiple [variants](/graphos/graphs/#variants) to represent the different environments where your supergraph runs (production, staging, and so on).
  - Optionally, [protect your production variant](/graphos/graphs/studio-features#protected-variants-enterprise-only) to avoid accidental changes while working in GraphOS Studio.
...
- For security, [disable introspection](/router/configuration/overview/#introspection) for all production GraphQL servers.
  - You can continue to view and fetch your GraphQL schemas from GraphOS Studio.
...
- If your subgraph servers are listed as [compatible with `FEDERATED TRACING`](/federation/building-supergraphs/supported-subgraphs/), ensure that you've enabled [federated traces](/federation/metrics/), and that you can view operation metrics as expected in GraphOS Studio.
...
- Ensure that you've [load-tested your graph](/technotes/TN0035-load-testing-graphql/).
  ...
  - To investigate performance issues, use GraphOS Studio to identify which operations are performing slowly.
