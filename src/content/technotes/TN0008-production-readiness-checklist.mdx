...
- Ensure your [clients are being correctly identified](/studio/client-awareness/) by name and version
  ...
  - For an example of enforcing this as a requirement at the Gateway, see [this technote for Client ID enforcement](/technotes/TN0001-client-id-enforcement/) 
...
- Consider adding caching layers
  - Apollo Server and Apollo Router have support for [Automatic Persisted Queries (APQ)](/apollo-server/performance/apq/) out of the box
    ...
    - Utilizing the [`@cacheControl` directive](/apollo-server/api/plugin/cache-control) can allow your CDN to cache APQ GET requests using the `Cache-Control` header
  ...
  - [Full response caching](/apollo-server/performance/caching/) can be introduced to improve performance out of the box
