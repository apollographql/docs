...
Here we are providing an example of queries/mutations which identify the actor attempting to use the graph.
...
Then by separating the mutation and query identities we are able to; separate (a) actions that can be performed on identities from (b) queries which can be performed on identities.
