...
description: GraphQL interfaces enable schema fields to return one of multiple object types, all of which must implement that interface.
...
Because interfaces can enforce this requirement on implementing object types, it can be tempting to create interfaces _purely_ to enforce field definitions. The following schema requires a `name` field on all its object types by declaring that those types implement the `Nameable` interface:
