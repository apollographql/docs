...
[Migrating to Apollo Router from @apollo/gateway](https://www.apollographql.com/docs/router/migrating-from-gateway)
...
[Configuring the subgraph fetcher](/federation/api/apollo-gateway#configuring-the-subgraph-fetcher)
...
In general, look at increasing memory limits as Node.js is single-threaded and cannot use more than one CPU. Additionally, we recommend keeping CPU utilization below 50% if possible.

At higher CPU utilization percentages, you may start to see event loop lag increase relative to usage which negatively impacts performance. Monitoring both metrics can help provide a clearer image of bottlenecks beyond just code profiling alone.

Additionally, we recommend keeping CPU utilization below 50%. At higher CPU utilization percentages, you may start to see event loop delays increase relative. Monitoring both metrics can help provide a clearer image of bottlenecks beyond just code profiling alone.

If CPU throttling becomes a problem, consider removing the limits on your gateway pods. More information on why you would do this can be found [here](https://erickhun.com/posts/kubernetes-faster-services-no-cpu-limits/).
