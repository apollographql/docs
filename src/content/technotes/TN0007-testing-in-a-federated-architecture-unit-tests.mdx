...
We recommend unit testing all your resolvers. When unit testing your resolvers, we recommend mocking as much of the data as possible using a package like [`@faker-js/faker`](https://www.npmjs.com/package/@faker-js/faker). This package allows you to use realistic fake data when mocking inputs and outputs.
...
### `__resolveReference` and integration testing subgraphs
...
Composition testing is specific to federated architectures. It involves testing that your graph will compose and handle the queries that are currently sent to it by clients. This is handled through [Rover](/docs/rover/). These checks are done by running the [`rover subgraph check`](/rover/commands/subgraphs#checking-subgraph-schema-changes) command. This is best used when configured within your CI/CD pipeline.
