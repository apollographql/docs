...
The following code defines two main components: `QueryResultProvider` and `useQueryResult`.
...
In this example, the `QueryResultProvider` component is being used to wrap the `Users` component, providing it with the `GET_USERS` query. The `Users` component then uses the `useQueryResult` hook to access the data, loading state, and error from the query context.
