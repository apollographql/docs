---
title: Enabling HTTP Defer/Subscriptions with the GraphOS Router and an API Gateway
subtitle: Enable HTTP multipart responses when using an API gateway in front of the router
...
Organizations can require their APIs—including [the router](/technotes/TN0037-api-gateways)—to be fronted by an API gateway. API gateways often aren't configured to support [long-lived connections over HTTP multipart](/router/executing-operations/subscription-support#http-callback-setup): API gateways by default usually buffer HTTP responses, but subscriptions and `@defer` over HTTP multipart require multipart responses to be streamed, not buffered.

The rest of this article describes how to configure API gateways from different vendors to stream and not buffer HTTP responses, therefore supporting responses for `@defer` and Federated Subscriptions over HTTP multipart.
