...
subtitle: Run, configure, and integrate schema checks into your CI pipeline
description: Learn how to run GraphQL schema checks against schema changes—including as part of your CI pipeline—and customize your them using Apollo GraphOS Studio or Rover CLI.
---

This guide covers running your first schema check, integrating checks into your CI pipeline, and configuring checks for your organization's use cases.
...
Each change to the schema is labeled either `PASS` or `FAIL`. For a complete list of schema change codes, see the [Reference](./checks-reference#schema-change-types).
...
Learn about other [check settings](#configuring-checks).
...
The Apollo Studio GitHub app enables GraphOS to send a webhook back to your GitHub project on each call to `rover subgraph check`. This provides built-in pass/fail status checks on your pull requests:
...
See [Connect to GitHub](./github-integration) for installation details.
