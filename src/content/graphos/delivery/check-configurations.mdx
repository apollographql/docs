...
- **Variants**: _Include_ all distinct operations that were executed against each selected variant of your graph. The default value is "Base variant" (that is, whichever variant schema checks are being run against).
...
You can customize a single run of schema checks by providing options to the Rover CLI. If you've _also_ [configured default rules](#using-graphos-studio-recommended) for schema checks, any command-line options you provide take precedence over those rules.
...
You can provide values for both of these flags. If you do, an operation must meet or exceed _both_ thresholds for schema checks to include it.
