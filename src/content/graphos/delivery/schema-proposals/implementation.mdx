...
GraphOS provides [schema checks](../schema-checks) to help you identify breaking changes. You [run checks on local changes](../run-checks#the-checks-lifecycle) before publishing them.
...
You can [configure schema checks](./configuration/#configure-schema-checks) that run in GraphOS to include a **Proposals** task. Proposals checks verify that the changes a check is running on have matching and approved schema proposals.

<img
  src="../../img/schema-proposals/proposal-check-settings.jpg"
  className="screenshot"
  alt="Proposals check settings in GraphOS Studio"
/>
