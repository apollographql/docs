...
This article describes actions in the **Proposal creation** stage of the [schema proposal process](./schema-proposals#proposal-process).

<SchemaProposalCreationMermaid />

## Start a proposal

You can create a new proposal in [GraphOS Studio](https://studio.apollographql.com?referrer=docs-content) from a graph's proposals list. To access the proposals list page, either:
  ...
  - From a particular graph, open the **Proposals** page from the left navigation.
...
1. Click the **Propose changes** button in the top right. A dialog entitled **Propose schema changes** appears.
   <img
     ...
     className="screenshot"
     alt="Schema proposals dialog in Studio"
     width="500px"
   />
2. Enter a descriptive **proposal title** that outlines the schema changes you plan to propose.
   ...
   If you want to edit the proposal's title later, you can do so from its [**Overview** tab](../review/#overview).

   </Note>

3. Select a **source variant**. The source variant's schema is used to start the proposal, but the proposal isn't kept in sync with any schema changes that occur while the proposal is in progress.

   <Tip>

   Because development environments are where most changes and experimentation occur, **Apollo recommends creating schema proposals from development variants**.
...
4. Optionally, enter the rationale for the proposed changes. This rationale appears as your proposal's README on the proposal's overview and gives collaborators and reviewers context for the proposed changes. This field accepts [Markdown](https://www.markdownguide.org/getting-started/).

    <Note>

    If you want to edit the proposal's README later, you can do so from its [**Overview** tab](../review/#overview).
...
To lint the schema changes you've made, click the Lint schema icon in the top right of the editor.
...
<Note>
 ...
 A violation line number may no longer exist because you've renamed or deleted a field. When this happens, the violation appears grayed out in the violations list. See, for example, `52 + User.token` in the screenshot above. The grayed out line indicates you can ignore the violation.
...
A dialog appears where you can optionally include a summary of your changes. This field accepts [Markdown](https://www.markdownguide.org/getting-started/).
...
**Approved** and **Implemented** statuses are automatically set once [necessary conditions](./schema-proposals#proposal-statuses) have been met.
