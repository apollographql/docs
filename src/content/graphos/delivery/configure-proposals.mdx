...
Schema proposal configurations are on a per-graph&mdash;not per-organization&mdash;basis, except for [muting notifications](#mute-notifications), which are on a per-teammate, per-proposal basis.
...
Once you've added [**default reviewers**](#default-reviewers), you can require _at least one_ of those approvals to come from a default reviewer by selecting **Require at least one default reviewer's approval**.
...
A modal appears where you can select to mute your notifications. To re-enable them, click the same icon.
