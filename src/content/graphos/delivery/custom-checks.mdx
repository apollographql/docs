...
Note that schema hashes are provided rather than the text contents because schema can be large.
To fetch a single supergraph or subgraph schema contents by hash, run the following GraphQL query against the Platform API. (See Platform API docs for instructions on how to make and authenticate requests to Platform API)
