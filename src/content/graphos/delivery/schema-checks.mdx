...
Your endpoint can then respond with any potential issues, which are displayed in GraphOS Studio along with other check result details.
...
For a guide on configuring checks, including enabling [custom checks](./custom-checks), see [Check Configurations](./check-configurations).
