...
[Custom, proposals, and contract checks](#types-of-checks) require an [Enterprise plan](https://www.apollographql.com/pricing/). All other schema checks are free as part of all Apollo plans.
...
Custom checks let you run custom validations, for example, enforcing custom linting rules or confirming schema changes belong to a particular git branch.
Once you [configure custom checks](./custom-checks), GraphOS sends schema change information to a specified webhook on each schema check run.

Your endpoint can then respond with any potential issues, which GraphOS Studio displays along with other check result details.
