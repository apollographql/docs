...
<td>
Run custom validation, for example, to enforce custom linting rules or confirm
schema changes belong to a particular git branch.
</td>
</tr>
<tr>
<td>
...
This article provides further details on build, operations, and custom checks.
...
Custom checks let you run custom validations using webhooks.
Once you [configure custom checks](), GraphOS triggers your specified webhook on each schema check run.
Specifically, GraphOS sends your webhook a payload with details of the schema changes submitted to the check. Your endpoint can then respond and identify any potential issues.
...
To learn how to run your first schema check, integrate checks into your CI pipeline refer to [Run Schema Checks](./run-checks).
For a guide on configuring and customizing checks, see [Check Configurations](./check-configurations).
