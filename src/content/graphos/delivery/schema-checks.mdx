...
With the exception of [contract checks](#types-of-checks), schema checks are **free** as part of [all Apollo plans](https://www.apollographql.com/plans/).

## Types of checks

GraphOS can perform the following types of schema checks:

- [**Build checks.**](#build-checks) For supergraphs, verify whether your proposed changes to a subgraph schema will successfully compose with your _other_ subgraph schemas.
...
- **[Contract checks](./contracts/)** (enterprise only). When running schema checks on a source variant, also check whether your proposed schema changes will break any downstream contract variants.

<blockquote>

**Most of this article covers composition and operation checks.**

- For details on contract checks, see [Contracts](./contracts/).
...
> Additionally, if you run a check within a Git repository, `rover subgraph check` sends both the commit hash and that hash's author to GraphOS to display on the check. If you want to override these values, [Apollo Rover can use environment variables to do so](/rover/configuring/#overriding).

### Rerunning checks

You can rerun checks from GraphOS Studio:
...
When you run `rover subgraph check`, GraphOS first performs a **build check** to verify that the subgraph schema still contains valid GraphQL after any changes. Build checks are also known as a **composition checks** because they verify that changes to the subgraph are compatible with your _other_ subgraph schemas, enabling them to compose into a _supergraph_ schema for your router ([learn more](/federation/federated-types/composition/)).

> If a build check fails, Studio does _not_ perform additional checks for the provided schema.

From your graph's Checks page in GraphOS Studio, you can click on a particular build check to view its result. If composition succeeded, you can view the composed supergraph schema. Regardless of success, you can view the proposed subgraph schema.

## Operation checks

If a [build check](#build-checks) succeeds, GraphOS then validates schema changes with **operation checks**. These checks use your graph's historical client operation data to determine whether any clients would be negatively affected by the proposed schema changes. For example, the changes might include removing a field that multiple clients currently include in their queries.
...
In cases like this, you can **override** a flagged change in GraphOS Studio from the associated check's details page:
