...
When you run `rover subgraph check`, GraphOS first performs a **composition check** to verify that the subgraph schema still contains valid GraphQL after any changes. Build checks are also known as a **composition checks** because they verify that changes to the subgraph are compatible with your _other_ subgraph schemas, enabling them to compose into a _supergraph_ schema for your router ([learn more](/federation/federated-types/composition/)).

> If a composition check fails, Studio does _not_ perform additional checks for the provided schema.
