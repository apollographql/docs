...
> Additionally, if you run a check within a Git repository, `rover subgraph check` sends both the commit hash and that hash's author to GraphOS to display on the check. If you want to override these values, you can [set environment variables in Rover](/rover/configuring/#overriding) to do so.
