...
When you run `rover subgraph check`, GraphOS performs a **build check** _before_ it performs any other checks. Also known as a **composition check**, a build check verifies that changes you make to a subgraph schema are valid GraphQL definitions and are compatible with your _other_ subgraph schemas, enabling them to compose into a _supergraph_ schema for your router ([learn more](/federation/federated-types/composition/)).

> If a build check fails, Studio does _not_ then perform additional checks for the provided schema.
