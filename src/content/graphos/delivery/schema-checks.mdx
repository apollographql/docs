...
- [Custom checks](#custom-checks): Run custom validation on your own systems alongside each schema check.
...
On each run of a schema check, GraphOS triggers a webhook you specify with a payload containing information about what was submitted into the check. Your endpoint can then respond and indicate which parts of a submitted schema may have issues that need to be addressed.
...
```graphql
query schema($graphId: ID!, $supergraphHash: SHA256, $subgraphAHash: SHA256, $subgraphBHash: SHA256) {
  graph(id: $graphId) {
...
```graphql
mutation SchemaCheckCallback($input: CustomCheckCallbackInput!, $name: String!, $graphId: ID!) {
  graph(id: $graphId) {
    id
    variant(name: $name) {
      id
