...
Additionally, if you run a check within a Git repository, `rover subgraph check` will include both the commit hash and that hash's author to GraphOS to display on the check. If you would like to override these values, [Apollo Rover can use environment variables to do so](/rover/configuring/#overriding).
