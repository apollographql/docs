...
> Contracts are available only for [Enterprise plans](https://www.apollographql.com/pricing/).

Apollo Studio **contracts** enable you to deliver different subsets of your supergraph to different consumers. Each contract filters specific portions of your supergraph's schema into a different Studio variant:
...
A contract uses one of your graph's existing [variants](/graphos/graphs/overview/#variants) (called the **source variant**) to generate its contract schema. If your source variant uses Federation 1, you need to enable its support for the `@tag` directive in Apollo Studio.
...
> Whenever Studio composes your source variant's [supergraph schema](/federation/federated-types/overview), that schema retains all of the `@tag`s from your subgraph schemas (this is different from most other directives, which are removed by default).
...
- **If you enabled the feature to hide unreachable types**, the contract schema _excludes_ objects, interfaces, unions, inputs, enums, and scalars _unless_ they are tagged with an _included_ tag.
  - A type is considered unreachable when:
    - There does not exist a path from a [root operation type](https://spec.graphql.org/October2021/#sec-Root-Operation-Types) to the type itself.
    - There does not exist a path from a [executable-directive](https://spec.graphql.org/October2021/#ExecutableDirectiveLocation) to the type itself.
...
> Studio might encounter an error while generating your contract schema. For descriptions of these errors, see [Error types](#error-types).

### 6. Use your new contract variant

Congratulations! You've created a contract in Apollo Studio. You can now use your contract variant to provide a [contract router or contract documentation](#what-are-contracts-for) to your users.
...
If you make any changes to the contract's included or excluded tags, Studio automatically runs [contract checks](#contract-checks) to determine whether those changes will affect any clients that user your contract variant.
...
| `UNREACHABLE_TYPE_MASKING`                 | Studio encountered an error while attempting to exclude unreachable types in the resulting contract schema.                         |
