...
- The proposal process **leverages GraphOS [schema checks](./schema-checks)&mdash;including [schema linting](./schema-linter)&mdash;at every stage**.
...
Before describing the details of this proposal process, it's helpful to understand the stages a proposal progresses through.

### Proposal stages

| Stage             | Description                                                                                                                                                                                                    |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Draft             | Default stage upon creation and until the proposal is ready for review.                                                                                                                                        |
...
| Implemented       | All the proposal's changes have been [published](./publishing-schemas). <br/> <br/> - Implemented proposals cannot receive further revisions <br/> - Their stage cannot be updated.                            |
| Closed            | Signals the proposal is suspended or abandoned. <br/> <br/> - Closed proposals cannot receive further revisions. <br/> - You can reopen a proposal by setting the stage to **Draft** or **Open for feedback**. |
...
These proposal stages enable the following end-to-end schema change management process:
...
graph LR
    ...
    Creation -->  DraftStage --> Edit --> Request
    end
    class DraftStage secondary;
    class DraftStage CircleSize50;
```
...
graph LR
    ...
    direction LR
    Feedback((Open for <br/>feedback <br/> stage))
...
graph LR
    ...
    direction LR
    ApprovedStage((Approved <br/> stage))
    Fetch["Fetch proposal and <br> implement locally"]
    Push["Run checks on <br> local changes"]
    Check[["GraphOS <br/> schema checks"]]
    ApprovedStage --> Fetch --> Push --> Check
    end
    class ApprovedStage secondary;
    class ApprovedStage CircleSize50;
...
graph LR
    ...
    direction LR
    Checks[["GraphOS schema <br/> checks pass"]]
    Publish["Publish schema <br/> to GraphOS"]
    Monitor[["GraphOS matches <br/> changes to proposals"]]
    ImplementedStage((Implemented <br/> stage))
    Checks --> Publish --> Monitor --> ImplementedStage
    end
    class ImplementedStage secondary;
    class ImplementedStage CircleSize50;
