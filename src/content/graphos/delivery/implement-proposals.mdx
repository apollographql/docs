...
When an **Approved** or **Open for feedback** proposal's changes have been fully implemented—[whether from one launch or many](#matching-changes-to-proposals)—the proposal's stage changes to **Implemented**.
...
- A proposal with **Draft** stage can't become **Implemented**, even when all its changes are published. Once such a proposal's stage is changed to **Open for feedback**, it automatically switches to **Implemented**.
- **Implemented** proposals cannot receive further revisions, nor their stage changed.
