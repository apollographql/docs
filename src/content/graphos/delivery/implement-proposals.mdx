...
Once the changes are implemented—including updating resolvers and other supporting work—you can use the [schema checks workflow](#validate-changes-with-schema-checks) to ensure that your organization only publishes changes that were part of an approved proposal.
...
Configuring the **Proposals** task's severity to **Error** ensures that only changes that have gone through the proposal workflow are published. You can set the severity to **Warning** if you want to be able to publish changes that haven't gone through the workflow but receive warnings about them.
...
When an open proposal's changes have been fully implemented—whether from one launch or many—the proposal's status changes to **Implemented**.
...
- **Implemented** (and **closed**) proposals cannot receive further revisions.
