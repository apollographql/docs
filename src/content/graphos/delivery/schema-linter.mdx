...
> ⚠️ The `rover subgraph lint` and `rover graph lint` commands validate all linter rules _except_ for [composition rules](./linter-rules#composition-rules). Use [`rover supergraph compose`](/rover/commands/supergraphs#supergraph-compose) to validate composition rules locally.
