...
You can create two kind of customized rules:
- Rules to require or forbid specific prefixes and suffixes in element names
- Rules to require descriptions on specific elements

You can create customized rules for the following elements:
...
> ⚠️ If any of the strings you enter conflict with an existing rule, the modal won't allow you to create it. For example, if you create a rule to require a capitalized prefix (for example, ) for field names, it conficts with the [`FIELD_NAMES_SHOULD_BE_CAMEL_CASE`](./linter-rules/#field_names_should_be_camel_case) rule, and you won't be able to save it.

GraphOS dynamically generates the rule name based on your selections. For example, if you create a rule forbidding certain prefixes for fields, the resulting rule name is `FIELD_FORBID_USER_DEFINED_PREFIX`.

<img
  className="screenshot"
  src="../img/schema-rules/new-rule-modal.jpg"
  alt="Linter rule customization in GraphOS Studio"
  width="700"
/>

You can add as many rules as you want by clicking **Add another rule**. When you're done, click **Create**.

Your customized rules appear with a **CUSTOM** tag in the rule list. You can edit a customized rule by clicking the pencil icon to the right of its name:
...
As with built-in linter rules, you can set a customized rule's [severity level](./schema-linter/#setting-severity-levels). To delete a rule, click the pencil icon and then click the trash can icon next to it in the customized rule modal.
