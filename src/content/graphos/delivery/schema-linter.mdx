...
> ⚠️ Schema linting is currently in **private preview**.
...
- You can configure how each variant of your graph treats violations of each linter rule.
    - ⚠️ **Variant-level configuration is not available during the private preview period!**
- You can also define **graph-wide linter defaults** for any variants you _don't_ configure individually.
    - Only these are available during the private preview period.
...
4. **To configure graph-wide linter defaults** for _all_ of your variants, click the **This Graph** tab.
    ...
    **To configure just the current variant,** remain on the **This Variant** tab.
      ...
      - ⚠️ **Variant-level configuration is not available during the private preview period!** All variants use graph-wide linter defaults.
...
> ⚠️ **During the private preview period,** the **Use Graph Settings** toggle is _always_ enabled for _all_ variants.
...
> **For the best experience running linter checks, install v0.15 or later of the Rover CLI.** Earlier versions of Rover can't correctly output the results of linter checks (but they do correctly exit with a nonzero code if a linter check fails).

### One-off linting

> ⚠️ **One-off linting requires v0.15 or later of the Rover CLI.** [Install the latest version.](/rover/getting-started)
