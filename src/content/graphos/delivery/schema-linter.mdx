...
- If you set up [schema checks](./schema-checks/) for your graph, linting runs as a separate check type alongside build and operations checks.
...
If you set up [schema checks](./schema-checks) for your graph, schema linting automatically runs as a separate check type alongside build and operations checks. You can view the results of all check types from your graph's **Checks** page in Studio.
For example, the screenshot below shows successful build and operations checks with a failed linting check.
