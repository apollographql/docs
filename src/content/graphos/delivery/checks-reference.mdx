...
This reference describes the operation checks statuses you may see in the Studio UI and the schema change types reported by [the Rover CLI `subgraph check` response](./check-configurations#the-check-response).

## Operation check statuses

GraphOS classifies [operation checks](./schema-checks#types-of-checks) in Studio with the following statuses:
