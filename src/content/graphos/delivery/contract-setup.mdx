...
subtitle: Learn how to create, edit, and validate contracts in GraphOS
description: Learn how to use the @tag directive in subgraph schemas and create, edit, and validate contracts in GraphOS Studio
---
...
1. If you're using the `@apollo/gateway` library instead of the Apollo Router:
...
2. If you're using Apollo Server:
...
After you've added tags, update your source variant by publishing your updated subgraph schemas to GraphOS.
...
1. Open your source variant's **Settings** page and open the **This Variant** tab. Open **Contracts** in the left nav:
...
In Apollo Federation 2, if you want to exclude a type or field from your source variant's API schema and all of its contract schemas, you can use the `@inaccessible` directive instead of `@tag`. For details, see [Using `@inaccessible`](/federation/federated-types/sharing-types/#using-inaccessible).

</Tip>

6. When you're finished adding tag names, click **Generate Preview**. Studio attempts to generate a contract schema based on the filters you provided and displays the result:
...
7. After you generate the preview, can click **Review** to continue to the next step.
8. You can now review all of the details of your contract:
...
9. If everything looks right, click **Create**. This kicks off the generation of your contract variant and its initial contract schema as a [launch](./launches).
...
If you want failing contract checks to fail your entire checks workflow, you can set **blocking downstream variants** configuration for your source variant. If contract checks fail for any of these specified contract variants, the source variant's checks workflow _does_ fail.
