...
When you run schema checks on a source variant, contract checks execute alongside the other checks for that variant ([build checks](./schema-checks/#build-checks) and [operation checks](./schema-checks/#operation-checks)).
