...
This reference lists the default rules that you can enforce with [GraphOS schema linting](./schema-linter/), along with the code that GraphOS returns for each rule violation. You can also create [customized rules](#customized-rules) to:
- forbid or require prefixes or suffixes you define
- require descriptions for specific schema elements
...
Refer to the schema linter page for [customized rule configuration instructions](./schema-linter#configure-customized-rules).

#### Prefix/suffix rules

These rules have the following naming convention: `{ELEMENT_NAME}_{FORBID|REQUIRE}_USER_DEFINED_{PREFIX|SUFFIX}`, for example, `INTERFACE_FORBID_USER_DEFINED_PREFIX`.
...
#### Rules to require descriptions

These rules have the following naming convention: `{ELEMENT_NAME}_REQUIRE_DESCRIPTION`

You can require the following elements to have descriptions:
- The entire schema
- All fields
- Only query and mutation fields
- Types
- Objects
- Interfaces
- Inputs
