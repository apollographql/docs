...
This reference lists the default rules that you can enforce with [GraphOS schema linting](./schema-linter/), along with the code that GraphOS returns for each rule violation. You can also create [custom rules](#custom-rules) to forbid or require prefixes or suffixes you define on specific schema elements.
