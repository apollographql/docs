...
This reference describes the rules that you can enforce with [GraphOS schema linting](./schema-linter/), along with the code that GraphOS returns for each rule violation.
...
Field names of the `Query` and `Mutation` types should never include HTTP verbs (`get`, `post`, and so on).
...
```graphql
enum EnumResidenceType { # highlight-line
  HOUSE
  APARTMENT
  CONDO
}
...
```graphql
enum ResidenceType { # highlight-line
  HOUSE
  APARTMENT
  CONDO
}
...
```graphql
enum ResidenceTypeEnum { # highlight-line
  HOUSE
  APARTMENT
  CONDO
}
...
```graphql
type Product {
  title: String @deprecated(reason: "Use name instead") #highlight-line
  name: String!
}
```
...
The `@tag` directive should always use a GraphOS-registered value for its `name` argument.
