...
description: Create your first Apollo supergraph
---
...
**If you don't have either, that's okay!** You can still try out GraphOS with an example server. To do so, provide all values labeled **Example setup** in the steps below.
...
**If you already have an Apollo account,** and you created your organization prior to 4 October 2022, you might need to **create a new organization** to create cloud supergraphs:
...
[Learn about upgrading an existing organization.](./org/plans#upgrading-to-a-current-plan)
...
You now have an Apollo Studio organization, but it doesn't contain any graphs! Let's create your first **cloud supergraph**, which will incorporate your existing GraphQL API.
...
1. From your new organization in [Apollo Studio](https://studio.apollographql.com/), navigate to the **Supergraphs** tab if it isn't already open.
...
8. Click **Next**. The following dialog appears:

    <img
     src="./img/supergraph-variant.jpg"
     class="screenshot"
     alt="Setting a variant for a new supergraph"
     width="400"
    />

    Every graph in GraphOS has one or more **variants**. Each variant represents a different environment where the graph runs (such as staging or production).

9. Specify a name for your supergraph's first variant.

    - Studio provides a default suggestion of `main`. Other common variant names include `production`, `staging`, `development`, and `test`.
    - You'll be able to create more variants later!

10. Click **Create Supergraph**.

Nice work! You've created your first cloud supergraph, which is now listed in your organization's **Supergraphs** tab:
...
Now that you've created your supergraph's first variant, let's look at the helpful views that Studio provides for it.

From your **Supergraphs** tab, click the name of the variant you just created. (In the screenshot above, that's **main**.) This opens your variant's README page.
...
| Page | Description |
|------|-------------|
| **README** | Displays the variant's editable Markdown-formatted README, along with other important details about the variant (such as its graph ref and router URL). [Learn more](./studio-graph-features/#the-readme-page) |
| **Schema** | Documentation for all your supergraph's types and fields. Supports filtering definitions by name, subgraph, and other values. [Learn more](./studio-graph-features/#the-schema-page) |
| **Explorer** | A powerful IDE for building and executing operations against your supergraph. [Learn more](./explorer/explorer) |
| **Subgraphs** | Displays the details for each separate GraphQL API that's part of your supergraph. (Each API is called a **subgraph**.) [Learn more](./cloud-routing/overview) |
...
| **Checks** | Displays the results of schema checks that have been run for the variant. [Learn more](./schema-checks) |
| **Launches** | Displays the results of recent launches executed for your supergraph. A launch occurs whenever changes are made to one of your subgraph schemas, resulting in an update to your router. [Learn more](./launches) |
...
Go to your variant's **Settings** page, then click **Cloud Routing**:
...
> For advanced CORS options, see [Configuring cloud routing](./cloud-routing/configuration#cors-settings).
...
On the **Cloud Routing** page, click **Save a secret**. The following dialog appears:
...
From your organization's **Supergraphs** tab, select your new variant and then open its Explorer page from the left navigation. The Explorer looks like this:
...
Here's a powerful benefit of a cloud supergraph: because clients execute operations against your GraphOS-managed router, the router automatically collects metrics on those operations! You can then view those metrics in Studio.
