...
subtitle: Upgrade your components in the recommended order to avoid errors
description: This tutorial shows how to update your federated GraphQL API safely. Upgrade your components in the recommended order to avoid errors.
---
...
- The **GraphOS Router** is powered by a particular version of the [Apollo Router Core](/router/).
...
- If you have a [self-hosted supergraph](./#self-hosted-supergraphs), you [download the latest Apollo Router Core binary](/router/quickstart#1-download-and-extract-the-apollo-router-binary) in every environment where your router runs.
...
- If you have a [cloud supergraph](./#cloud-supergraphs), GraphOS automatically updates your router to the latest supported version whenever that version changes.
  ...
  - You can see which router version you're running from the **Cloud Router** section of your variant's Settings page in [Studio](https://studio.apollographql.com/?referrer=docs-content):
...
For details on each router version's corresponding Apollo Federation version, see [this article](/router/federation-version-support#support-table).
...
1. Go to your variant's **Settings** page, then find the **General > Build Pipeline** section.

2. Click **Upgrade**.

3. In the dialog that appears, select a Federation version from the dropdown and click **Save**.

GraphOS immediately initiates a new build using the selected Federation version. All future builds also use the selected version.
Successful build completion triggers a launch that delivers an updated schema to your router.

### Composing with the Rover CLI

If you host your own GraphOS Router instance, you can use the [Rover CLI](/rover/) to compose its supergraph schema via [the `rover supergraph compose` command](/rover/commands/supergraphs#composing-a-supergraph-schema).
...
- Each version of the GraphOS Router is compiled against a particular version of Federation 2 (for details, [see this table](/router/federation-version-support)). Any given router version is backward compatible with previous versions of Federation. However, it isn't compatible with newer versions.
