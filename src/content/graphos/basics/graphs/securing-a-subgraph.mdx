...
subtitles: Restrict access to your supergraph using secrets
...
This article describes steps for securing your subgraphs when using a [cloud supergraph](/graphos/cloud-routing).
...
To restrict subgraph communication to only your router, we recommend creating a separate shared secret for each of your subgraphs. Whenever your router queries a subgraph, it includes that subgraph's corresponding secret in an HTTP header.
...
For details, see [Managing secrets](/graphos/cloud-routing/configuration#managing-secrets).
