...
To shut down your local supergraph, use `CTRL+C` to stop the _first_ `rover dev` process you started. This automatically terminates all _secondary_ `rover dev` processes that are part of the same session.

<Note>

If you stop a _secondary_ `rover dev` process, your session remains active, and the router's schema is automatically updated to reflect the removed subgraph.
