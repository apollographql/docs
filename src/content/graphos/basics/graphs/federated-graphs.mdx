...
- **Deleting a subgraph can break existing clients!** This action is equivalent to removing _all_ the subgraph's types and fields from your supergraph (unless they're also defined in _another_ subgraph). If a client sends an operation that includes any of those types and fields, the router will _reject_ that operation.
