...
The Apollo Router is a high-performance routing runtime packaged as a [standalone binary](https://github.com/apollographql/router). You can choose to host your own Apollo Router:
...
Since the Apollo Router is [source-available](https://www.apollographql.com/blog/evaluating-apollo-router-understanding-free-and-open-vs-commercial-features), you can use the router's codebase without connecting it to GraphOS. If you self-host the router, you can [connect it to GraphOS](./quickstart/self-hosted#6-connect-the-router-to-graphos) for additional functionality. Cloud routers are automatically connected to GraphOS.

A router connected to GraphOS, regardless of whether it's cloud- or self-hosted is called a **GraphOS Router**, and has access to [specific GraphOS features](../enterprise#graphos-router-features) depending on the connected GraphOS organization's plan.
