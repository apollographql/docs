...
With [GraphOS Enterprise](../enterprise/), you can enhance your graph's security by maintaining a **persisted query list** (**PQL**) for your supergraph's self-hosted router. This list should contain _all_ of the trusted GraphQL operations that your your app clients execute against your router:
...
## Benefits of registered persisted queries

Your router can use its persisted query list (PQL) to **protect your supergraph** and **speed up your clients operations**:

- When you enable **operation safelisting**, your router _rejects_ any incoming operations that aren't registered in its PQL. Safelisting blocks unauthorized requests from unauthorized clients.
...
graph LR;
  ...
  webClient & androidClient & iosClient -->|Queries|router
  router <-->|Verifies queries are registered|pql
  router -->|Routes registered <br/>queries to your graphs|api
```
...
Compared to GraphOS registered persisted queries, APQ has a few limitations:

|                   | Automated persisted queries | Registered persisted queries |
| ----------------- | --------------------------- | ---------------------------- |
| Performance       | ✅ Clients can send identifiers instead of full query strings reducing request sizes dramatically and latency.                         |✅ Clients can send identifiers instead of full query strings reducing request sizes dramatically and latency.     |
| Ready on startup   | ❌ With APQ, one of your router instances must receive any given operation string from a client at least once, in order to [cache it](/router/configuration/distributed-caching).        |✅ Your router fetches its full PQL from GraphOS on startup, which means clients can _always_ execute operations by their PQL-specified ID.       |
| Safelisting       | ❌ APQ doesn't provide safelisting capabilities because the router dynamically populates its APQ cache over time with _any_ operations it receives.        |✅ Clients pre-publish their operations to GraphOS. Your router fetches its PQL on startup, enabling it to reject operations that aren't present in the PQL.      |

If you _only_ want to improve request latency and bandwidth usage, APQ probably addresses your use case. If you _also_ want to secure your supergraph with operation safelisting, you should use GraphOS registered persisted queries.
...
> When you first adopt persisted queries, you should always start with the _least_ restrictive security level ([performance-only](#performance-only)). You can then enable increasingly restrictive levels after you make all necessary changes to your clients.
...
- [Performance with logging](#performance-with-logging): Clients can still optionally execute operations by providing a PQL-specified ID, but the router also logs any unregistered operations.
...
There are three main steps for implementing persisted queries for safelisting:
 ...
 If you want to safelist operations, you will eventually need to implement tooling in all of your client apps. However, you can incrementally adopt registered persisted queries by starting with the [performance-only](#performance-only) mode.

 Continue reading for details on each implementation step, or skip to the [incremental adoption section](#incremental-adoption-path) for the recommended strategy for incremental adoption.
