...
With [GraphOS Enterprise](../enterprise/), you can enhance your graph's security by maintaining a **persisted query list** (**PQL**) for your supergraph's self-hosted router. Your router checks incoming requests against the PQL, which acts as a safelist of trusted operations made by your first-party apps.
...
Your router can use its persisted query list (PQL) to **protect your supergraph** and **speed up your clients operations**:

- When you enable **operation safelisting**, your router _rejects_ any incoming operations that aren't registered in its PQL.
...
The Apollo Router also supports a related feature called [automatic persisted queries](/router/configuration/in-memory-caching#caching-automatic-persisted-queries-apq) (APQ). With APQ, clients can execute a GraphQL operation by sending the hash of its operation string, instead of sending the full string.

APQ has a few limitations compared to preregisted persisted queries.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td> Queries are registered at <b>runtime</b>. One of your router instances must receive any given operation string from a client at least once, in order to <a href="/router/configuration/distributed-caching">cache it</a>.</td>
      <td> Clients contribute to the PQL at <b>build-time</b>. Your router fetches its full PQL from GraphOS on startup, which means clients can <em>always</em> execute operations by their PQL-specified ID.</td>
    </tr>
    <tr>
      <td>Safelisting</td>
      ...
      <td>✅ Clients pre-publish their operations to GraphOS. Your router fetches its PQL on startup, enabling it to reject operations that aren't present in the PQL.</td>
    </tr>
  </tbody>
</table>
...
You can find details on how to configure these in the router in the [implementation section](#implementation).

These varying levels of permissiveness allow you to incrementally adopt safelisting on an app-by-app basis. Specifically, you can use audit mode until you are confident that all of your apps' trusted operations have been registered in the PQL. This way, you don't have to worry about turning on safelisting and accidentally rejecting one app's trusted operations.

Refer to the [incremental adoption section](#incremental-adoption-path) for a comprehensive guide.

## Implementation

There are three main steps for implementing persisted queries for safelisting:

  1. Creating a PQL and registering it
  2. Configuring your router to use the PQL
  3. Updating your client apps to send ids for registered queries

Your PQL list should eventually contain _all_ of the trusted GraphQL operations that your your app clients execute against your router.

#### PQL creation and registration
...
However, you can incrementally adopt registered persisted queries by starting with the [performance-only](#performance-only) mode. Continue reading for details on each implementation step, or skip to the [incremental adoption section](#incremental-adoption-path) for the recommended strategy for incremental adoption.
...
### Persisted query list registration
...
#### Generate client operation manifests
...
> When you first adopt persisted queries, you should start with the _least_ restrictive security level ([performance-only](#performance-only)). You can then enable increasingly restrictive levels after you make update your clients.

The Apollo Router supports the following security levels, in increasing order of restrictiveness:

- [Performance-only](#performance-only): Clients can optionally execute an operation on your router by providing the operation's PQL-specified ID.
    - All other levels also provide this core capability.
    - This level provides no operation safelisting.
- [Audit mode](#audit-mode): Clients can still optionally execute operations by providing a PQL-specified ID, but the router also logs any unregistered operations.
    - The level helps you identify operations you still need to register before turning on safelisting.
...
Turning on logging is a crucial step for gauging how ready your client apps are for safelisting. The logs identify which operations you need to either add to your PQL or stop your client apps from making. 
...
> ⚠️ **Before you apply this configuration,** first make sure your PQL contains _all_ of the GraphQL operations that _all active versions_ of your clients execute. If you enable safelisting _without_ ensuring this, your router will reject any unpublished client operations!
