...
> **Note:** You can use the performance-only security level with [automatic persisted queries](/router/configuration/in-memory-caching#caching-automatic-persisted-queries-apq) enabled.
...
preview_persisted_queries:
  ...
  log_unknown: true
```
> **Note:** You can use audit mode with [automatic persisted queries](/router/configuration/in-memory-caching#caching-automatic-persisted-queries-apq) enabled.
