...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td> Clients contribute to the PQL at <b>build-time</b>. Your router fetches its full PQL from GraphOS on startup, meaning clients can always execute operations using their PQL-specified ID.</td>
    </tr>
    <tr>
      <td>Safelisting</td>
      ...
      <td>✅ Clients pre-publish their operations to GraphOS. Your router fetches its PQL on startup, enabling it to reject operations not present in the PQL.</td>
    </tr>
  </tbody>
</table>
...
- **Safelisting with query strings allowed**: The router _rejects_ any incoming operations that aren't present in its PQL.
    - Before moving to this security level, ensure _all_ of your client operations are present in your PQL.
- **Safelisting with IDs only**: The router rejects any incoming operations that aren't present in its PQL, _and_ clients _must_ execute an operation by providing its PQL-specified ID.
    - Before moving to this security level, ensure _all_ of your clients execute operations by providing their PQL-specified ID.

You can find details on configuring the router's security level in the [implementation section](#implementation).

These levels of permissiveness allow you to incrementally adopt safelisting on an app-by-app basis. Specifically, you can use audit mode until you're confident that all your apps' trusted operations have been registered in the PQL. That way, you don't have to worry about turning on safelisting and accidentally rejecting one app's trusted operations.
