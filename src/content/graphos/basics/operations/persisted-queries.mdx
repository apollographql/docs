...
When generating operation IDs for a manifest, you should use a value that's unique to each operation, such as the query document's cryptographic hash. Apollo's [manifest generation tools](#31-generate-persisted-queries-manifests) use the base16 representation of the document's SHA256 hash, which is the same format used for [APQ](#differences-from-apq).
...
> Whitespace and other formatting treatment will change before this feature reaches [general availability](/resources/product-launch-stages/#general-availability). See [operation formatting considerations](#operation-formatting-considerations) for details.
...
#### Operation formatting considerations
...
Specifically, operations that differ only in whitespace, comments, and other ignored tokens will be considered the same. Similarly, operations that only differ in the order of their top-level operation and fragment definitions will be considered to be the same. Other differences, such as the order of fields or arguments, will still cause operations to be considered as different.

These changes will make creating manifest generation tools more straightfoward. Client libraries often combine operation and fragment definitions in a nondeterministic order, which can cause complexities for manifest generation tools.