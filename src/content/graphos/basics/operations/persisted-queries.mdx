...
- If you enabled [safelisting](#safelisting), the router rejects _any_ operations you deleted from the PQL but that your clients still perform.
  - This is true whether clients send full operation strings or operation IDs for the deleted operation.
- When [allowing operation IDs](#allow-operation-ids) or in [audit mode](#audit-mode-dry-run), if a client sends the full operation string of a deleted operation, the router executes it.
