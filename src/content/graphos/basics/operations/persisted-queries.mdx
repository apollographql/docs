...
|**Auditing**| Executing operations by providing a PQL-specified ID is still optional, but the router logs any unregistered operations.|
...
These levels of permissiveness allow you to [incrementally adopt](#incremental-adoption-path) persisted queries on a client-by-client basis. Specifically, the router should use the auditing security level until you're confident that _all_ your clients' trusted operations have been registered in the PQL. Refer to the [incremental adoption section](#incremental-adoption-path) for a step-by-step guide.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Ensure tooling uses the correct versions.</td>
      <td>Platform team</td>
    </tr>
    <tr>
...
If you have multiple clients and apps, you can approach these steps iteratively, completing each for one client at a time. While doing this, you should use the auditing—or "dry run"—[security level](#security-level).

Once your PQL has registered all your clients' operations and you've ensured your client apps are only sending trusted operations, you can update your security level to the safelisting security level.

Continue on for each implementation step's details, or skip to the [incremental adoption section](#incremental-adoption-path) for the recommended for incremental adoption strategy. (This section assumes you have a high-level understanding of each implementation step.)
...
2. Follow all [implementation steps](#implementation-steps) for your chosen client:
    ...
    1. When [configuring the router](#2-router-configuration), start with either [performance-only](#performance-only) or [auditing](#auditing-dry-run) security level.
    1. [Update your chosen client](#3-client-updates) to use only trusted operations, either by full query string or operation ID.
    1. If you haven't already, [configure your router](#2-router-configuration) to use the [auditing](#auditing-dry-run) security level. Use the router logs to confirm that your the client is only sending trusted operations.
