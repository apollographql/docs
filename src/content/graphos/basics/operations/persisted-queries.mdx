...
Since every operation should [have a unique ID](#id), you can't update existing operations. (If you try to publish a different operation to an existing ID, Rover returns an error.) Instead, you can only add new operations and delete existing ones.
...
If you want to delete an operation, you can do so from Studio. From the persisted query lists page, click a persisted query list to open it. Then, click the **Actions** button next to a particular operation to delete it.
