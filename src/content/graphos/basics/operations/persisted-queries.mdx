...
|**Auditing**| Executing operations by providing a PQL-specified ID is still optional, but the router also logs any unregistered operations.|
...
These levels of permissiveness allow you to incrementally adopt safelisting on an app-by-app basis. Specifically, you can use logging until you're confident that _all_ your apps' trusted operations have been registered in the PQL. That way, you don't have to worry about turning on safelisting and accidentally rejecting one app's trusted operations. Refer to the [incremental adoption section](#incremental-adoption-path) for an overview.
...
When you first adopt persisted queries, you should start with a less restrictive security level—either [performance-only](#performance-only) or [auditing](#auditing-dry-run). You can then enable increasingly restrictive levels after you update all your clients.
...
Persisted queries' [tiered security levels](#security-levels) let you adopt an incremental approach rather than simultaneously requiring all clients to query via trusted operations IDs. You can follow these steps for incremental adoption:
...
2. Follow all [implementation steps](#implementation-steps) for your chosen client:
    1. Begin by [creating a PQL in GraphOS](#11-pql-creation).
    1. When [linking the PQL](#12-link-the-pql-to-variants) to a variant, start with a testing or staging  environment.
    1. Use the [client-specific tooling](#generation-methods) to generate an [operation manifest](#13-generate-client-operation-manifests) as part of your client's CI/CD pipeline.
    1. [Publish the manifest](#14-publish-manifests-to-the-pql) to the PQL.
    1. When [configuring the router](#2-router-configuration), start with either [performance-only](#performance-only) or [auditing](#auditing-dry-run) mode.
    1. [Update your chosen client](#3-client-updates) to use only trusted operations, either by full query string or operation ID.
    1. If you haven't already, [configure your router](#2-router-configuration) to use [auditing](#auditing-dry-run) mode. Use the router logs to confirm that your the client is only sending trusted operations.
    1. [Link the PQL](#12-link-the-pql-to-variants) to a production variant.
...
If safelisting is your goal, you'll need to [coordinate across client teams](#coordinate-with-client-teams) to complete these steps for each of your client apps. You may need to toggle between linking staging and production variants as you test each of your clients operations.

Once your router's logs are clear of unexpected operations, you can configure your router to use [safelisting mode](#router-security-levels). Then, update your client apps to use IDs rather than full query strings.
...
2. Communicate to your client development teams that adopting persisted queries will require adding tooling to their CI/CD pipeline.
    - Specifically, each client team will need to perform the following steps of this adoption path: [Generate client operation manifests](#13-generate-client-operation-manifests) and [publish manifests to the PQL](#14-publish-operations-to-the-pql)
