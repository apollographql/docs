...
- Client apps can execute an operation by providing its PQL-specified ID _instead of_ a full operation string.
    - Querying by ID can significantly reduce latency and bandwidth usage for very large operation strings.
    - Your router can _require_ clients to provide operations by ID and reject full operation strings (even operation strings in the PQL).
...
1. Ensure your router instances are ready to work with PQLs:
    ...
    - Ensure your router instances are [connected to your GraphOS Enterprise organization](/router/enterprise-features/#enabling-enterprise-features) and that they're associated with a variant that your PQL is linked to.
...
If you want to use this security level, you should always _first_ set up [safelisting with operation strings allowed](#safelisting-operation-strings-allowed). This level requires modifying _all_ your clients to execute operations via PQL-specified ID instead of an operation string. While making those necessary changes, you can use the less restrictive safelisting level in your router.
...
- [Apollo iOS](/ios/fetching/peristed-queries/)
- [Apollo Kotlin](/kotlin/advanced/persisted-queries/)

The Apollo Client Web requires you to use an [additional package](https://www.npmjs.com/package/@apollo/persisted-query-lists) at runtime to send persisted queries. You use this package along with the [existing APQ client link](/react/api/link/persisted-queries). This package is required because certain aspects of the operation sent at runtime may not match the registered operations without it. Mobile clients have a more deterministic approach to formatting operations and, thus, don't need it.
...
2. Follow all [implementation steps](#implementation-steps) for your chosen client:
    ...
    1. (Optional) Update your chosen client to use only trusted operations, either by full operation string or operation ID.
        - Teams working with Apollo Client Web also need to use an [additional package](https://www.npmjs.com/package/@apollo/persisted-query-lists) at runtime to send persisted queries.
...
This string will be the query document executed when a client sends the corresponding ID. If you are using "safelisting with operation strings allowed" or "audit mode", **this string must _exactly_ match the query document that your client sends your router, including whitespace and field order.** We intend to relax this requirement slightly before this feature reaches General Availability. For details, see [Ensuring consistent operation signatures](#ensuring-consistent-operation-signatures).
...
> Note: We intend to make the freeform GraphQL comparison performed by the safelist (and audit mode) less strict before this feature reaches General Availability. Specifically, we intend to consider operations that differ only in whitespace, comments, and other ignored tokens to be the same, and operations that only differ in the ordering of their top-level operation and fragment definitions to be the same. (Other ordering differences such as the order of fields or arguments will still be considered different.) This will make it easier for manifest generation tools, as client libraries often combine operation and fragment definitions found in different locations in the project source code in a nondeterministic order.
