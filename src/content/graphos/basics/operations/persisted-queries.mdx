...
**Never use an operation's _name_ for its PQL ID.** Different clients (or even different versions of the _same_ client) might execute different operations with the same name, and _all_ those distinct operations should be present in your PQL.
...
Prior to Router v1.28, safelisting required the incoming document to match the document in the safelist _precisely_, including white space, comments, and top-level definition order.
