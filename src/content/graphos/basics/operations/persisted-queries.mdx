...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><a href="#3-client-updates"> (Optional) <br/> 3. Client app updates </a> </td>
      <td>
        This step provides performance benefits but <em>isn't necessary for safelisting.</em>
        <br/>
        <br/>
        <ul>
          <li>
...
Platform teams also need to create a PQL in GraphOS Studio so that client teams can register trusted operations to it.
Part of PQL creation is linking it to the intended graph variant(s).
...
> It's safe to link an empty or incomplete PQL to a variant because your router doesn't _use_ its PQL for anything until you [configure it to do so](#2-enable-safelisting) (covered in a later step).
...
After client teams [publish operations](#12-publish-manifests-to-the-pql) to your PQL, platform teams can configure router instances to fetch and _use_ your PQL.
...
This enables your clients to execute operations by providing their PQL-specified ID instead of the full query string.
Your router also continues to accept full query strings, even for operations that _don't_ appear in its PQL.

The router works with both query formats since the PQL-specified ID is the query string's [SHA256 hash](https://en.wikipedia.org/wiki/SHA-2), and the router can derive it from the query string if needed. 
...
### (Optional) 3. Client updates

With your [PQL published](#1-pql-creation-and-registration) and your [router configured](#2-enable-safelisting), you can begin to update your clients to use trusted operations IDs. You can do this one client at a time as you [publish client-specific PQMs](#14-publish-manifests-to-the-pql) to the PQL.
...
2. Follow all [implementation steps](#implementation-steps) for your chosen client:
    ...
    1. When [configuring the router](#2-enable-safelisting), start with either [performance-only](#performance-only) or [auditing](#auditing-dry-run) security level.
    1. [Update your chosen client](#3-client-updates) to use only trusted operations, either by full query string or operation ID.
    1. If you haven't already, [configure your router](#2-enable-safelisting) to use the [auditing](#auditing-dry-run) security level. Use the router logs to confirm that your the client is only sending trusted operations.
...
By generating identifiers based on query documents this way, you ensure that different operations always have different IDs. This prevents unexpected collisions in your PQL. It also allows the router to execute queries both by full query strings and PQL-specified IDs, since the router can derive the ID from the query string if needed. 
