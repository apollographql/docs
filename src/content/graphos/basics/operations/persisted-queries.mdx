...
For example, most applications treat the following queries equivalently, but the router will _reject_ the client operation because it doesn't match the PQL entry exactly:
...
Differences in whitespaces and comments also cause the router 
...
query GetBooks {
  books {
    title
    publishDate
    /* Both this comment and the
    following difference in whitespace
    would cause this operation to fail. */
    #highlight-start
    author { name }
    #highlight-end
  }
