...
For example, most applications treat the following queries equivalently, but the router would reject the client operation because it doesn't match the PQL entry exactly:
...
Differences in whitespaces or comments between a registered operation and the operation a client sends also cause the router to reject client operations. 
...
If you're building your own manifest generation tool, ensure it accounts for any such operation changes in your chosen client library. Otherwise, the router will reject your app's operations due to an operation string mismatch.

Similarly, if your clients execute operations by providing their PQL-specified ID, they might execute an operation with a different shape than expected if you don't account for these operation changes.