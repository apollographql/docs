...
For example, if you've enabled [safelisting](#safelisting), the router rejects any operations you deleted from the PQL, but that your clients still perform. This is true whether clients send full operation strings or operation IDs for the deleted operation.

While deleting options cannot be directly undone via the Studio UI, you can always republish a deleted operation using [the Rover CLI](#32-publish-manifests-to-the-pql).
