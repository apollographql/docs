...
To simplify troubleshooting connection issues to AWS VPC Lattice, we recommend you enable subgraph error inclusions for your graph variant by setting `include_subgraph_errors`. This will allow you to see error messages generated by your subgraphs.
...
3. In the left sidebar menu, click on **Settings**, then on **Cloud Router** to access the GraphOS Cloud settings page.
...
4. In the **Router configuration YAML** section, ensure that the following configuration block that sets `include_subgraph_errors` is present:
...
This will trigger a new launch. Please wait for a few minutes for your Cloud Router to update with this new configuration. You can monitor the deployment status in the Launches page for your graph variant.

Once you have identified and resolved the underlying issue, we recommend you to disable subgraph errors by removing the block that sets `include_subgraph_errors` and saving the Router configuration YAML again.
