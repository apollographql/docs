...
With [GraphOS Enterprise](../enterprise/), you can maintain a **persisted query list** (or **PQL**) for your supergraph's self-hosted router. This list should contain _all_ of the known GraphQL operations that _all_ of your app clients execute against your router:
...
Setting up persisted queries with operation safelisting is a multi-step process that requires setting up tooling for _all_ of your client apps. Based on the availability of your client teams, it might take days or even weeks for you to get fully up and running with safelisting.
...
1. Communicate to all of your client development teams that adopting persisted queries will require adding tooling to their CI/CD pipeline.
...
2. Identify which team members will assist with adding tooling to their respective CI/CD pipelines.
...
To generate an operation manifest with Apollo Kotlin, you modify your project's [Gradle plugin configuration](/kotlin/advanced/plugin-configuration) to generate a manifest instead of standard Kotlin source for your operations:
