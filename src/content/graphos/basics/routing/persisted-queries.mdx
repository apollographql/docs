---
title: Trusted operations with GraphOS
...
With [GraphOS Enterprise](../enterprise/), you can register a **trusted operation list** for your supergraph's router. This list combines _all_ of the known GraphQL operations that _all_ of your app clients execute against your router.

A trusted operation list (or **TOL**) can protect your supergraph _and_ speed up your clients:

- You can configure your router to _reject_ incoming operations that aren't in its TOL.
- Client apps can execute an operation by providing its TOL-specified ID _instead of_ providing the full operation string.
    - For very large operation strings, this can dramatically improve latency and bandwidth usage.
    - You can even _require_ that operations be provided via TOL-specified ID, rejecting query strings even if they match a trusted oepration.
