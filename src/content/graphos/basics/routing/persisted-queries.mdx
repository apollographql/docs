...
Router instances with a persisted query list (PQL) can use that PQL in one of a few different modes, which you set in your router's [YAML config file](/router/configuration/overview/#yaml-config-file).

These modes include:

- [Performance-only](#performance-only-no-safelisting): The router supports clients executing an operation by providing its PQL-specified ID.
    - All other modes also provide this core capability.
    - In this mode, the router performs no operation safelisting.
...
- [Safelisting with IDs only](#safelisting-ids-only): The router rejects any incoming operations that aren't present in its PQL, _and_ clients _must_ execute an operation by providing its PQL-specified ID.
    - In this mode, the router rejects all requests that provide a full GraphQL operation string, _even if that operation is present in the PQL._
...
When generating operation identifiers for a manifest, you should use a value that's derived directly from the query's details, such as the query document's cryptographic hash. Apollo tools that extract operations use the base16 representation of the document's SHA256 hash, which is the same format used for [APQ](#differences-from-apq).

By generating identifiers based on query documents this way, you ensure that different operations always have different IDs. This prevents you from unintentionally overwriting an existing entry in your PQL with an entirely different operation (which would break any clients that execute the original operation).

## Managing PQLs

### Removing an operation from a PQL

You might occasionally need to remove an operation from a PQL because the router should no longer allow it. Organization members with the **Graph Admin** role can remove individual operations via your graph's PQL page in GraphOS Studio.
...
> **In almost all cases,** you should publish a _new_ operation to your PQL instead of modifying an existing one.
