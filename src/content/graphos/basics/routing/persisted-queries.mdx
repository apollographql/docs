...
description: Safelist known operations and minimize request latency
---
...
With [GraphOS Enterprise](../enterprise/), you can register a **persisted query list** (or **PQL**) for your supergraph's self-hosted router. This list contains _all_ of the known GraphQL operations that _all_ of your app clients execute against your router:
...
graph LR;
  ...
  webClient & androidClient --->|Registers<br/>operations|pql
  router -.->|Fetches PQL|pql
```

Using a PQL, your router can **protect your supergraph** and **speed up your clients**:

- You can configure your router to _reject_ any incoming operations that aren't in its PQL.
    - In this case, the router uses its PQL for **safelisting**.
- Client apps can execute operations by providing their PQL-specified IDs, _instead of_ providing full operation strings.
    ...
    - Your router can even _require_ that clients provide operations this way, rejecting full query strings even if they're present in the PQL.
...
- APQ doesn't provide operation safelisting capabilities, because the router populates its operation cache over time with _any_ operations it receives.
    - **With GraphOS persisted queries,** trusted operations are pre-registered with GraphOS. Your router fetches its PQL on startup, enabling it to approve or reject operations based on their presence in the PQL.
...
To use persisted queries, you first create at least one **persisted query list** (**PQL**) in GraphOS Studio. Each PQL is associated with a single supergraph in GraphOS, and you can apply a single PQL to multiple _variants_ of a supergraph.
...
Any time after you create a PQL, you can apply it to one or more variants of your supergraph. It's safe to apply an empty or incomplete PQL, because your router doesn't _use_ its PQL until you configure it to do so (covered in a later step).

Apply PQLs from your supergraph's PQL page in Studio:
...
After you apply a PQL to a variant, GraphOS validates it against the variant's reported operation history and flags any recent operations that aren't represented in the PQL.

GraphOS then uploads the PQL to Uplink, the service that delivers configuration to your router at runtime.

> As a best practice, you should apply PQLs to your staging _and_ production environments. To help with debugging in staging, you can configure your staging router to _log_ unrecognized operations instead of _rejecting_ them.

### 3. Register operations to the PQL

Any time after you create a PQL, you can start registering GraphQL operations to it. **A PQL should contain _all_ of the GraphQL operations that _all_ of your trusted clients execute on your associated router.**

You register operations to a PQL using the [Rover CLI](/rover/):
...
You can incorporate this registration into your CI/CD pipeline, enabling you to automatically register new operations whenever a new client app version is released.

#### Generating a manifest file

As shown in the example above, you provide `rover persisted-queries publish` a JSON **manifest** that contains details for all of the operations you want to register.
