...
1. Update your Apollo Router instances to version `1.24.0` or later. [Download the latest version.](../quickstart/)
...
After you create a PQL, you can link it to one or more variants of your graph. It's safe to link an empty or incomplete PQL, because your router doesn't _use_ its PQL for anything until you configure it to do so (covered later).
...
After you create a PQL, you can start publishing GraphQL operations to it.

> A PQL should contain _all_ of the GraphQL operations that _all active versions_ of your clients execute on your associated router. If you enable safelisting _without_ ensuring this first, some of your clients will break!

You publish operations to a PQL by providing a JSON **manifest** to the [Rover CLI](/rover/):
...
You can incorporate the publishing process into your CI/CD pipeline, enabling you to automatically publish new operations whenever a new client app version is released.
...
During this preview period, Apollo Client for Web provides a mechanism for generating a manifest file from your application source. Similar mechanisms for other client libraries are in development.
...
This example manifest file includes three operations:
...
When you [publish operations](#3-publish-operations-to-the-pql) to a persisted query list (PQL), you provide those operations to the Rover CLI in a JSON **manifest** file.

Currently, Apollo Client Web and Apollo iOS each provide a mechanism for generating these manifest files directly from your application source.

> If your client uses another GraphQL client library, you can create your own mechanism for generating manifests. See [Manifest format](#manifest-format).
