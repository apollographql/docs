---
title: GraphOS quickstart for self-hosted supergraphs
...
Hello! 👋 This quickstart gets you up and running with GraphOS and a self-hosted supergraph. **None of this setup requires a paid plan.**
...
A **_self-hosted_ supergraph** is a supergraph with a router that you host and manage in your _own infrastructure_. This is in contrast to a [**cloud supergraph**](./getting-started/), which uses a router that's hosted and managed by Apollo.

We'll cover these concepts in more detail as we proceed through the steps below. Let's jump in!
...
Complete the first step of <a href="./getting-started/#1-create-your-apollo-account" target="_blank">Get started with GraphOS</a> (**Create your Apollo account**), then return here.
...
A **graph ref** uniquely identifies a particular variant of a registered graph in GraphOS. Every graph ref is a string with the following format:
...
Next, let's do the same thing for our `reviews` subgraph, **again substituting your graph ref where indicated**:
...
Go to your variant's Operations page in Studio, which looks like this:
...
The test queries you executed in the previous step should already be represented on this page! After you update all of your clients to query your router, the Operations page becomes vital to monitoring your supergraph's performance.
...
Next, we'll cover some of the most common and important actions to perform on your supergraph, including:
