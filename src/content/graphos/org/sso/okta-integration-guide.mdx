...
- A GraphOS user account with [to-do: role]
...
- Share your Okta app integration's SAML metadata with Apollo via GraphOS Studio.

The SSO setup wizard in GraphOS Studio guides you through these steps.

### Create a custom Okta app integration

1. Go to [GraphOS Studio](https://studio.apollographql.com?referrer=docs-content). Open the **Settings** page from the top navigation. Open the **Org management** tab from the left sidebar and click **Enable SSO**.

   <Note>

   If you are migrating from SSO you setup before April 2024, you still need to click **Enable SSO**. The **Update domain** button is only for those who've already setup SSO with this self-serve method and need to update it.

   </Note>

2. Follow the prompts in the setup wizard to enter your SSO details.
3. Once you get to **Step 2: Your IdP and Apollo** in the wizard, open your Okta Administrator Dashboard in a separate browser tab.
4. In your Okta Administrator Dashboard, go to the **Applications** view and click **Create App Integration**.
...
5. In the dialog that appears, select **SAML 2.0** as your sign-in method. Click **Next**.
6. The **Create SAML Integration** dialog appears. In the **General Settings** step, provide the following values:
...
7. In the **Configure SAML** step, provide the following values:
   ...
   - **Audience URI (SP Entity ID)**: Entity ID provided by by the setup wizard
   - Leave the default values for other settings, including leaving the **RelayState** blank.

8. Still in the **Configure SAML** step, scroll down to **Attribute Statements**. Set values for the following attributes:
...
9. In the **Feedback** step, select **I'm an Okta customer adding an internal app**. Click **Finish**.
10. Back in the setup wizard in GraphOS Studio, click **Continue** to share SSO metadata with Apollo.

### Share SAML metadata with Apollo

1. In your Okta Administrator Dashboard, in the Okta SAML app integration you just created, go to the **Sign On > Settings > SAML 2.0 > Metadata details** section.
...
2. Copy and paste the contents of the **Metadata URL** text box into the setup wizard in GraphOS Studio and click **Continue**.
3. GraphOS Studio populated your SSO metadata based on the **Metadata URL** you entered. Verify the values are correct and click **Continue**.
4. Click **Continue** to enable SSO in GraphOS Studio.
