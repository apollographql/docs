...
You can [use Okta's official GraphOS Studio integration](#using-oktas-official-graphos-studio-integration) (recommended) or create a custom SAML integration (legacy).
...
## Using Okta's official GraphOS Studio integration

### Supported features

The [Okta GraphOS Studio SAML](https://www.okta.com/integrations/graphos-studio/) integration currently supports the following features:
...
You can use Okta's [Bookmark App integration](/#2-add-graphos-studio-as-a-bookmark-app) to simulate an Identity Provider-initiated (IdP-initiated) flow to allow users to sign in from Okta.

### 1. Add and configure the integration in Okta
...
2. Search for "GraphOS Studio" and select it. Then, click **+ Add integration**.
   ...
   <img
     ...
     alt="Adding the GraphOS Studio integration in Okta"
     class="screenshot"
     width="500"
   />

3. In the **General Settings** tab that opens, select **Do not display application icon to users**. (You'll [set up a Bookmark App](#2-add-graphos-studio-as-a-bookmark-app) instead.) You can optionally change the **Application label** or keep the default "GraphOS Studio" label. Click **Done**.
...
4. The **Assignments** tab opens—you'll return to it later to [assign users](/#assign-users-in-okta) to the integration. For now, open the **Sign On** tab and copy the **Metadata URL** under **Metadata details**. Send the URL to your Apollo contact. They will complete your SSO setup.
