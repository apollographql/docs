---
title: Set up Apollo SSO with Okta
---
...
This guide walks through configuring Okta as your Apollo organization's identity provider (IdP) for SSO. Once you've set up your integration, you need to [assign users to it in Okta](#assign-users-in-okta) so they can access GraphOS Studio via SSO.
...
1. Create a custom Okta app integration for Apollo.
...
2. From your Okta Administrator Dashboard, go to the **Applications** view. Click **Create App Integration**. The following dialog appears:
...
5. In the **General Settings** step, provide the following values:
...
6. In the **Configure SAML** step, provide the following values:
   ...
   - **Default RelayState**: RelayState value provided by Apollo
   - Leave the default values for other settings.

7. Still in the **Configure SAML** step, scroll down to **Attribute Statements**. Set values for the following attributes:
...
8. In the **Feedback** step, select **I'm an Okta customer adding an internal app**, then click **Finish**.

### Step 2. Send SAML metadata to Apollo

1. From your new Okta SAML integration's details page, scroll down and click **View SAML setup instructions** on the right side:
...
Once your Apollo integration is set up, you need to assign users to it so they can access GraphOS. You can assign individual users or groups by following these steps:
...
When users are first assigned to the Apollo GraphOS integration, they must re-login to GraphOS Studio. Once you've confirmed the new configuration works as expected, remove any legacy Apollo integrations in Okta if you have them.
