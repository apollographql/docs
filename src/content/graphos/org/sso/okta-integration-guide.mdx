...
1. Go to [GraphOS Studio](https://studio.apollographql.com?referrer=docs-content). Open the **Settings** page from the top navigation. Open the **Org management** tab from the left sidebar and click **Enable SSO**. A setup wizard appears.
   ...
   If you are migrating from SSO set up before April 2024, you still need to click **Enable SSO**. The **Update domain** button is only for those who've already set up SSO with this self-serve method and need to update it.
...
In the setup wizard, click the **Enable SSO** button to finalize setup.
Once your SSO setup is finalized, you must [assign users](#assign-users-in-okta) to your custom app in Okta to give them access.
