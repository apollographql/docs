---
title: Set up Apollo SSO with Microsoft Entra ID (formerly Azure AD)
---
...
This guide walks through configuring Microsoft Entra ID (formerly known as Azure Active Directory) as your Apollo organization's identity provider (IdP) for SSO.
...
1. Create an app registration for Apollo.
2. Send your Entra ID's app integration's SAML metadata to Apollo.

Setup requires administrator privileges.

### Step 1. Create an app registration

1. Send a request to your Apollo contact for Apollo's service provider (SP) SAML information. Include the organization name(s) you are setting up SSO for. Your Apollo contact will respond with the following values:
   ...
   These values differ for each GraphOS organization. If setting up SSO for multiple organizations, ensure you repeat these steps for each organization using different values.

   </Note>

1. Go to your [Azure Portal](http://portal.azure.com/) and then navigate to **Microsoft Entra ID**.
1. In the left navigation, select **App Registrations**. In the App Registration page's top menu, click **+ New Registration**.
1. On the **Register an application** page, provide the following information:

   - Provide a descriptive name for the app, such as `Apollo GraphOS`.
   - Under **Supported account types**, select which Microsoft account types will have access.
   - Under **Redirect URI**, set **Web** as the platform and enter the [to-do]
     Then click **Register**.
...
1. Still in the **Overview** section, select **Endpoints** from the top menu. Copy and paste the **Federation metadata document** URL into the same local text file.
1. Open the **Certificates & secrets** section in the left navigation and click **+ New client secret** and create a new secret.
1. Copy and paste the secret's **Value** to the same text file you created earlier.
1. In the **API permissions** section of your app registration, check whether `User.Read` is listed by default. If isn't, add it manually:
...
1. Also in the **API Permissions** section, select **Grant admin consent for Default Directory**. Doing this ensures that your users don't need to grant consent during SSO.
...
1. From the **Manifest** section of your app registration, find the `groupMembershipClaims` property. Change its value from `null` to either `"All"` or `"SecurityGroup"`.
   ...
   This ensures that the group membership claim is included in the access token
   during SSO.
...
Send your Apollo contact your the information you saved to your local text file:
