...
1. Create an enterprise application for Apollo in Entra ID.
...
2. Go to your [Azure Portal](http://portal.azure.com/) and then navigate to **Microsoft Entra ID**.
3. On the default directory page, select **+Add > Enterprise application**.
...
6. On the app's **Overview** page, select **2. Set up single sign-on**.
7. On the app's **Single sign-on** page, select **SAML** as the single sign-on method.
8. At the top of the SAML-based Sign-on configuration page, click **Upload metadata file** and upload the file provided by your Apollo contact. Alternatively you can enter these values manually in the **Basic SAML Configuration** section:
...
9. In **Attributes & Claims** ensure the following **claim name**s have the corresponding **value**s:
...
10. Under **SAML Certificates** copy the **App Federation Metadata URL** into a text file for use in the next step.

### Step 2. Send SAML metadata to Apollo

Send your Apollo contact the **Federation metadata document URL** you previously saved. Your Apollo contact will complete your SSO setup.

## Assign users in Entra ID

Once your Apollo application is set up, you need to assign users to it so they can access GraphOS. You can assign individual users or groups from the **User and groups** page of your Apollo application. You may want to begin by adding yourself individually and then testing single sign-on by clicking **Test** at the bottom of the **Single sign-on** page.

Once you've successfully tested your own user's ability to single sign-on, add any applicable users or groups.
When users are first assigned to the Apollo GraphOS integration, they must re-login to GraphOS Studio. Once you've confirmed the new configuration works for your users, remove any legacy Apollo applications in Entra ID or app registrations in Azure AD if you have them.
