...
To share SSO configurations, each organization needs to individually configure SSO according to the latest instructions (as of April 2024) for their particular IdP.

- [Okta](./okta-integration-guide)
- [Azure AD](./azure-ad-integration-guide)
- [Generic SAML-based IdP](./saml-integration-guide)
...
When this feature reaches [GA](/resources/product-launch-stages#general-availability), it will also support service-provider-initiated (SP-initiated) SSO.
...
Therefore, during preview, all members of an organization with multi-org SSO configured must log in from their IdP. If they try to log in on `studio.apollographql.com/login`, they will receive an error message directing them to log in via IdP.
