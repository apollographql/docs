...
This guide walks through configuring SAML- or OIDC-based single sign-on (SSO) for an Apollo organization. Configuration requires:

- Administrative access to your identity provider (IdP)
- The [**Org admin** role](../members#organization-wide-member-roles) in GraphOS

If you want to [multiple organizations to share the same SSO](./multi-org), repeat the configuration steps for each organization.
