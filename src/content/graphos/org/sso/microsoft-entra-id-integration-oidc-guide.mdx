---
title: Set up OIDC SSO with Microsoft Entra ID (formerly Azure AD)
subtitle: Configure Entra ID as your GraphOS organization's identity provider
description: Configure Microsoft Entra ID (formerly Azure AD) as your Apollo GraphOS organization's SSO provider with this OIDC setup guide.
---
...
1. Create an app registration for Apollo GraphOS in Entra ID.
1. Retrieve your Entra client ID, client secret, and issuer.
1. Configure API permissions in Entra ID.
1. Send your Entra ID's app registration's OIDC metadata to Apollo.

Setup requires at least a [Cloud Application Administrator](https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/permissions-reference#cloud-application-administrator) role.

### Step 1. Create an Entra ID app registration

1. Send a request to your Apollo contact to configure an OIDC SSO integration. Include the organization name(s) you are setting up SSO for, as well as the email domains you want to associate with the organization. Your Apollo contact will respond with:
...
2. Go to your [Microsoft Entra admin center](https://entra.microsoft.com/). Alternatively, you can sign in to the [Azure Portal](http://portal.azure.com/) and then navigate to **Microsoft Entra ID**.
3. In Entra, go to **Identity > Applications > App registrations**. If accessing Entra from the Azure Portal, go to **Manage > App registrations**. Select **+New registration** in the top menu.
4. On the **Register an application** page, provide the following information:
...
2. Next to **Client credentials**, click **Add a certificate or secret** and create a new secret.
3. Copy and paste the secret's **Value** field to the same text file you created earlier.
4. Back in the **Overview** section, select **Endpoints** from the top menu.
5. Copy and open the **OpenID Connect metadata document** URL in and find the **"issuer"** value. It should have a format like `https://login.microsoftonline.com/unique-value/vx.x`. Copy and paste this into your text file.
...
3. From the **Manifest** section of your app registration, find the `groupMembershipClaims` property. Change its value from `null` to either `"All"` or `"SecurityGroup"`. This ensures that the group membership claim is included in _the_ access token during SSO.
4. **Save** your changes.
...
- **Discovery URL**: you don't need to enter this since the **OpenID Connect metadata document** URL follows the standard `/.well-known/openid-configuration` format

Once submitted, inform your Apollo contact. They will then be able to enable SSO.
...
Once your Apollo GraphOS application is set up in Entra ID, you need to assign users to it so they can access GraphOS. You can assign individual users or groups from the **User and groups** page of your Apollo GraphOS application in Entra ID.
...
<img
  src="../../img/sso/entra-sso-test.jpg"
...
When users are first assigned to the Apollo GraphOS application, they must re-login to GraphOS Studio. Once you've confirmed the new configuration works for your users, remove any legacy Apollo GraphOS applications in Entra ID or app registrations in Azure AD if you have them.
