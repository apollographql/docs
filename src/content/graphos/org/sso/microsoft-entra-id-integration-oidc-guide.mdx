...
3. Copy and paste the secret's **Value** to the same text file you created earlier.
4. Back in the **Overview** section, select **Endpoints** from the top menu.
5. Copy and open the **OpenID Connect metadata document** URL in a new browser tab. Find the **`issuer`** value. It should have a format like `https://login.microsoftonline.com/unique-value/vx.x`. Copy and paste this URL into your text file.
...
Once your SSO setup is finalized, team members who previously had access to GraphOS will need to log back in via the new SSO setup.
To give them access, you first need to assign them in Entra.
