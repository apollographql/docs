...
2. Go to your [Microsoft Entra admin center](https://entra.microsoft.com/). Alternatively, you can sign in to the [Azure Portal](http://portal.azure.com/) and then go to **Microsoft Entra ID**.
...
3. Copy and paste the secret's **Value** to the text file you created earlier.
4. Back in the **Overview** section, select **Endpoints** from the top menu.
5. Copy and open the **OpenID Connect metadata document** URL in a new browser tab. Find the **`issuer`** value. It should be formatted like `https://login.microsoftonline.com/unique-value/vx.x`. Copy and paste this URL into your text file.
...
Once your SSO setup is finalized, team members who previously had access to GraphOS must log back in via the new SSO setup.
...
2. Also from the **API Permissions** section, select **Grant admin consent** next to the **+ Add a permission** button. This ensures your users don't need to grant consent during SSO.
