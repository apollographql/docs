...
1. Request OIDC SSO data from Apollo.
1. Create an app registration for Apollo GraphOS in Entra ID.
1. Retrieve your Entra ID endpoint URL, client ID, and client secret.
1. Configure API permissions in Entra ID.
1. Send your Entra ID's application's OIDC metadata to Apollo.
...
### Step 1. Request OIDC SSO data from Apollo

Send a request to your Apollo contact to configure an OIDC SSO integration. Include the organization name(s) you are setting up SSO for, as well as the email domains you want to associate with the organization. Your Apollo contact will respond with the following:
...
1. Go to your [Microsoft Entra admin center](https://entra.microsoft.com/). Alternatively, you can sign in to the [Azure Portal](http://portal.azure.com/) and then navigate to **Microsoft Entra ID**.
2. Go to **Identity > Applications > App registrations** and select **+New registration** in the top menu.
3. On the **Register an application** page, provide the following information:
   ...
   - Under **Supported account types**, select which Microsoft account typesshould have access to GraphOS.
   - For **Redirect URI**, enter the OIDC redirect URL provided by your Apollo contact.

   Then click **Register**.

### Step 3. Retrieve app registration data
...
3. Copy and paste the **OpenID Connect metadata document** URL into the same local text file you previously created.
4. From the **Certificates & secrets** section of your app registration, click **+ New client secret** and create a new secret.
5. Copy and paste the secret's **Value** field to the same text file you created earlier.
...
### Step 5. Send OIDC metadata to Apollo

To complete the configuration, complete the Apollo-provided link provided in step 1 with the appropriate information and inform your Apollo contact. They will then be able to enable SSO.
