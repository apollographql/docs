...
| **Contributor**     | Can push new schemas to graphs, but cannot configure graph settings or integrations. Intended for backend developers who are authorized to make changes to graphs. Cannot push schema changes to [protected variants](../graphs/management/#public-variants).                                       |                        Enterprise only                         |
...
| **Consumer**        | <p>Has view-only access to the organization's graph schemas, but not metrics. Can also execute queries in the Explorer. Intended for application developers building clients against a graph.</p><p>This role is available to all plan types. For legacy Team plans, **Consumer** members do not count as billed users.</p> |                              All                               |
...
| Create graphs                                    |     ✓     |      ✓      |                                              ✓                                               |                       |          |
| Create new variants                              |     ✓     |      ✓      | Non-[protected](../graphs/management/#public-variants) variants only |                       |          |
| Push schemas to a graph                          |     ✓     |      ✓      | Non-[protected](../graphs/management/#public-variants) variants only |                       |          |
| Manage Explorer settings (URL, etc)              |     ✓     |      ✓      | Non-[protected](../graphs/management/#public-variants) variants only |                       |          |
