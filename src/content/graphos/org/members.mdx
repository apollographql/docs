...
There is an additional role called **Persisted Query Publisher** that is only available for graph API keys (not for organization members). This role is only available for API keys for graphs in Enterprise organizations. An API key with this role can run the `rover persisted-queries publish` command to publish to [persisted query lists](../operations/persisted-queries), but does not have any other read or write access to the graph. This role is appropriate for use in the deployment process by client development teams which need to be able to publish their operations to your graph's persisted query lists but should not be able to view your graph's schema, changelogs, or use other **Consumer**-level features.
