...
A **supergraph** (also known as a [federated graph](/federation/)) is a graph that's composed of one or more **subgraphs**:
...
Each [variant](./overview/#variants) of a Studio supergraph has its own subgraph schemas, supergraph schema, change history, and metrics.
...
When you [publish a subgraph's schema](../schema/cli-registration/), you can specify which variant of the supergraph you're publishing that schema to.

## Managing subgraphs

After you create a [cloud supergraph](./overview/#cloud-supergraphs) (or a new variant of one), you can view a list of its associated subgraphs from the variant's Subgraphs page:
...
You can add your new subgraph to your GraphOS supergraph in any of the following ways:

* [In Apollo Studio](#adding-a-subgraph-in-apollo-studio)
...
To add a subgraph to your supergraph with Rover, you publish the subgraph's schema! To do this, you use the exact same Rover command as [updating an _existing_ subgraph](#update-a-subgraph-schema). The only difference is that you provide the name of the _new_ subgraph to the command.

[Learn about publishing subgraph schemas.](../schema/cli-registration/#subgraph-schemas)

To add a new subgraph to a variant of your cloud supergraph, you [publish that subgraph's schema](../schema/cli-registration/) using the Rover CLI. In your command, make sure to specify the correct values for the following:

* The graph and variant to publish to
* The name for the subgraph
* The routing URL that your router will use to communicate with the subgraph

You can also click **Add a subgraph** on your variant's Subgraphs page to view instructions for publishing a subgraph schema.
...
- To provide the `@contact` directive to Apollo, you must publish your subgraph schemas by providing a local `.graphql` file as the `--schema` option of [`rover subgraph publish`](../schema/cli-registration/).
...
The [Apollo Studio Explorer](../explorer/explorer/) provides enhanced support for working with supergraphs:
