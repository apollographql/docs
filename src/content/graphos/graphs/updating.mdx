...
The method you use to update your routing layer depends on which type of supergraph you have:
...
- **If you have a [serverless cloud supergraph](./overview/#cloud-supergraphs),** GraphOS _automatically_ updates your routing layer to the latest supported version whenever that version changes.
    
    - You can see which Apollo Router version you're running from the **Cloud Routing** section of your variant's Settings page in [Studio](https://studio.apollographql.com/):
...
GraphOS will begin using the selected Federation version with the graph's next build.

### Composing with the Rover CLI

The [Rover CLI](/rover/) enables you to compose a supergraph schema via [the `rover supergraph compose` command](/rover/commands/supergraphs#composing-a-supergraph-schema). You _don't_ usually need to run this command if your router is connected to GraphOS.

If you _do_ use this command, it selects which version of composition to use based on the `federation_version` key in the YAML configuration file you pass:
...
federation_version: 2 #highlight-line
subgraphs:
  # ...
```

- If `federation_version` is `2`, the command uses the _very latest release of Federation_.
- If `federation_version` is `2.1`, the command uses the most recent _patch_ release of Federation 2.1.
- If `federation_version` is `=2.1.2`, the command uses _exactly_ Federation 2.1.2.
...
After both your [routing layer](#1-update-your-routing-layer) _and_ your [build pipeline](#2-update-your-build-pipeline) support a particular minimum version of Apollo Federation, your subgraphs can use features that were first introduced in that version.
