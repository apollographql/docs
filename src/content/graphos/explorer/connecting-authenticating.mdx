...
Similar to tools like Postman, the Explorer can run custom scripts when executing each GraphQL operation. These JavaScript-based scripts often populate the values of [environment variables](#environment-variables) that are then injected into HTTP header values or GraphQL variables. This can be especially useful for managing authentication flows like OAuth, for example by refreshing an expired access token, or validating response data. For an example, see [Operation Scripts](#operation-scripts).
...
- [**Operation scripts.**](#operation-scripts) If you use [operation collections](./operation-collections/), you can define a separate preflight and postflight script for each saved operation in a collection. These scripts can then control everything that happens before, during, and after an operation is executed (as long as a user individually [enables the scripts](#enabling-operation-scripts)).
  - Pre-operation scripts will be run before the explorer operation.
  - Post-operation scripts will be run after the explorer operation.
...
2. From the Explorer's bottom panel, select the Pre-Operation or Post-Operation Script tab and click **+ Add Script**:
...
3. Create, test, and save your script.
   ...
   To test your postflight operation script the operation script modal has a Mocked Operation Response area. A JSON graphql response can be added here for **Test script** to access with [`explorer.response.body`](#explorerresponsebody).
...
The JSON representation of the GraphQL operation response.
