...
The Explorer currently enables you to authenticate via [request headers](#request-headers), [cookies](#cookies), and [scripts](#scripting).
...
The Explorer's Settings tab includes a section for providing Environment Variables. Here, you can provide sensitive information that you can then include in header values or [scripts](#scripting). This enables you to share operations (including headers) with other team members _without_ exposing sensitive data.
...
Similar to tools like Postman, the Explorer can run custom **scripts** before each GraphQL operation.
The main purpose of scripting is to populate environment variables that can be used as header values or graphql variables. 
This can be especially useful for managing authentication flows like OAuth, for example by refreshing an access token.

There are two places in the Explorer to write these scripts. Each variant can have a **preflight script** and each operation in an [Operation collection](./operation-collections/) can have a **operation script**
...
<table class="field-table api-ref">
  <thead>
    <tr>
      <th></th>
      <th>Preflight Script</th>
      <th>Operation Script</th>
    </tr>
  </thead>
<tbody>
<tr>
...
Anyone who can view the variant can view script
</td>
<td>

Anyone who can view the collection the operation belongs to can view the script
</td>
</tr>
<tr>
<td>
...
Anyone who can edit the collection the operation belongs to can _create or edit_ the script
</td>
</tr>
<tr>
<td>
...
Preflight scripts default enabled/disabled status is set globally on the variant, users can choose to disabled it for themselves
</td>
<td>

Operation scripts are enabled per user, so each user needs to enable them in the explorer settings for them to run
</td>
</tr>
<tr>
<td>
...
Preflight scripts always execute first (before GraphQL operations and operation scripts)
</td>
<td>

Operation scripts execute after any Preflight Script thats defined, and before the GraphQL operation
</td>
</tr>
</tbody>
</table>

#### ⚠️ Important considerations for scripts

- All team members with access to a variant or operation the script is on can _view_ the script.
...
- Team members need to [enable](#enable-operation-scripts) the execution of operation scripts
- Scripts can use the `explorer.runOperation` method to execute saved operations which may also have scripts, and those can also call `explorer.runOperation`. These will all be chained together.
...
By default, a variant's preflight script runs automatically before every GraphQL operation that's executed in the Explorer.
Team members can temporarily disable the script from the **Scripts** in the **Personal Settings** section of the Explorer Settings tab.
...
#### Enable Operation Scripts

Operation Scripts are disabled by default, each user will need to go into the explorer settings and enable them. 
...
These symbols are available within the scope of a script. Snippets for each are available via the **Show snippets** link in the script dialog.
...
- If `scope` is `sandbox`, `graphRef` will default to `null`
- If `graphRef` is `null`, `scope` will default to 'sandbox'
