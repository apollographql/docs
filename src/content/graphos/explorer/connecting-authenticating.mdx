...
- [**Preflight scripts**](#preflight-scripts): You can define a _single_ preflight script _per variant_ of a graph. This script runs before _every Explorer operation_ that's executed against the associated variant.
...
- Preflight scripts are enabled by default. Individual team members can [turn them off](#disabling-preflight-scripts) for their own Explorer usage.
- Operations scripts are turned off by default. Individual team members can [enable them](#enabling-operation-scripts) for their own Explorer usage.
