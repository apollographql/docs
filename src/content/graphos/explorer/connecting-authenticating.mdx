...
The Explorer's Settings tab includes a section for providing Environment Variables.
Here, you can provide sensitive information that you can then include in header values or graphql variables, or [scripts](#scripting).
This enables you to share operations (including headers) with other team members _without_ exposing sensitive data.
...
Or in graphql variables values with double curly braces, as shown: 
...
Or in a script using `explorer.environment.get('token')`
