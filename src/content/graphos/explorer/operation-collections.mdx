...
<img
  ...
  alt="Explorer collections list"
  className="screenshot"
  width="300"
/>

Each [variant](../graphs/#variants) of a graph has its own set of operation collections. An operation collection can be either **personal** (visible only to you) or **shared** (visible to all organization members who can view the corresponding variant).
...
4. Click **+ Create collection** at the top of either the Personal or the Shared section, depending on which type of collection you want to create.
   ...
   <img
     src="../img/operation-collections/create-collection.jpg"
     alt="Create collection dialog"
     class="screenshot"
     width="400"
   />
...
1. Define at least one operation in the Explorer's editor panel:
   ...
   If you define _multiple_ operations in a single editor, _all_ those operations are saved together as a single entry in a collection.
...
2. Click the "floppy disk" button (shown in the screenshot above) to open the save dialog:
   ...
   <img
     src="../img/operation-collections/save-operation.jpg"
     alt="Dialog for saving operation to collection"
     class="screenshot"
     width="500"
   />
...
Non-admin organization members _can't_ edit another team member's shared collection, _unless_ that team member specifically configures the collection to allow this.
...
If you create a [public variant](../graphs/studio-features/#public-variants) of your graph, any shared collections for that variant are visible to _everyone_, not just members of your organization. Anonymous users can view and execute operations in a public collection, but they can't make any changes to the collection.

## Sandbox collections

[Apollo Sandbox](./sandbox/) is a special mode of GraphOS Studio that doesn't require an Apollo account. If you use Sandbox and _do_ sign in with your Apollo account, you can save personal operation collections that are _specific_ to Sandbox:
