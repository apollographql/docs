...
Each [variant](../org/graphs/#variants) of a graph has its own set of operation collections. An operation collection can be either **personal** (visible only to you) or **shared** (visible to all organization members who can view the corresponding variant).
...
<img src="../img/operation-collections/added-operation.jpg" alt="Saved operation shown in left panel" class="screenshot" width="350" />
...
Each graph variant in Studio has a [README page](../studio-graph-features/#the-readme-page) that you can customize to document that variant. You can embed a variant's shared collections and operations in its README using [shortcodes](../studio-graph-features/#readme-shortcodes) that are shown in the README editor.

### Public collections

If you create a [public variant](../graph-access-permissions/#public-variants) of your graph, any shared collections for that variant are visible to _everyone_, not just members of your organization. Anonymous users can view and execute operations in a public collection, but they can't make any changes to the collection.
