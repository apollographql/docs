...
If a team member has defined and turned on all possible scripts for a particular operation, the order is as follows:
...
By default, operation scripts are **off** for each individual team member. Individuals can turn on operation scripts from the Explorer's Settings tab:
...
Here is a small example that runs an operation and sets an environment variable for use in future operations:
