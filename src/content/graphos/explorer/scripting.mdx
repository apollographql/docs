...
The following example uses the [`explorer.oauth2Request`](#exploreroauth2request) method to authenticate using an OAuth 2.0 provider's URL. View the [reference](#exploreroauth2request) for more details.
...
See the following example for how you may

### Set an authorization environment variable

The following example uses the [`explorer.environment.set`](#explorerenvironmentset) method to set an authorization token based on the results of an authorization request to Okta with date based expiration.
