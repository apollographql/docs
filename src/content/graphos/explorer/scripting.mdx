...
You can use a post-operation script to filter, map, or otherwise transform response data and then log it. This can be helpful when a request returns numerous results in the response.
...
function exampleTransform(ships, manufacturerFilter) {
  // Filter the ships based on the manufacturer
  const filteredShips = ships.filter(ship => ship.manufacturer === manufacturerFilter);

  // Map the filtered ships to a new format
  const mappedShips = filteredShips.map(ship => ({
    id: ship.id,
    name: `${ship.name} (${ship.model})`, // Combine name and model
...
// Example: Filter ships manufactured by "Corellian Engineering Corporation" and map to a new format
const allShips = explorer.response.body.data.Ships;
const transformedShips = exampleTransform(allShips, "Corellian Engineering Corporation");
