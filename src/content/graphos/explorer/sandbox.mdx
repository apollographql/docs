...
You can also change the endpoint via a [URL parameter](#url-parameters).
...
You can customize a few things about Sandbox when first loading the page via URL parameters—this enables users to skip a few steps when loading Sandbox from a link. These are all the parameters available:
...
| `document`  | The GraphQL query that should be populated in the operation panel                                                            |
| `variables` | The variables for the query that should be set below the operation panel, must be a JSON object.                             |
| `headers`   | Headers that should be included in requests to the `endpoint`, populated below the operation panel, must be a JSON object.   |

For example, the following URL sets each of these parameters:
...
> Note that, for readability, the above URL is not URL-encoded. Most browsers will automatically encode the URL for you, but it is best practice to URL-encode the parameters before providing them to a user.

This URL results in Sandbox loading like the following screenshots:
