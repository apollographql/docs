...
A URI-encoded, serialized object containing initial HTTP shared header values to populate in the Sandbox on load.
Shared headers are sent with every introspection request, while regular headers are only sent with operations triggered by the user from Explorer.
