...
Set `endpointIsEditable` to `false` to disable users of your embedded Sandbox instance from changing the endpoint url.
</td>
</tr>
...
If you pass `hideCookieToggle` to true, users will default to your setting for `includeCookies`, but will be able to toggle this on and off.
...
You can find these values from a registered graph in Studio by clicking on the ... next to an operation
in the Explorer of that graph & selecting 'Copy operation & collection ids'.
...
Optional. Defaults to true. If you don't want the embedded Sandbox to poll against your `initialEndpoint` for schema updates, pass `false` here.
...
These headers are not editable by users of this Sandbox, and they are applied to every tab your users open. Users can choose to uncheck them.

These are the only headers that will apply to your introspection query in Sandbox.
