...
The Explorer supports file upload for servers that support the [graphql multipart request spec](https://github.com/jaydenseric/graphql-multipart-request-spec). To run an operation that requires files as input, use the **Add files** button on the **Variables** tab under the operation editor in Explorer. From here, you can set the name of the variable, and select the files you want to upload. Note that if your operation supports multiple files as a variable, you will need to switch on the toggle for that variable.
...
> Note that supergraphs do _not_ currently support subscription operations. You can only execute subscriptions in the Explorer with a [monograph](../graphs/#monographs).
