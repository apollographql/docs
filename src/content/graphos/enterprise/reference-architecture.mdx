...
- [build-a-supergraph-subgraph-a](https://github.com/apollosolutions/build-a-supergraph-subgraph-a) and [build-a-supergraph-subgraph-b](https://github.com/apollosolutions/build-a-supergraph-subgraph-b) - Template repositories for the subgraphs used in the reference architecture
...
subgraph changes are published via CI/CD. Additionally, the router is able to push performance and utilization metrics to GraphOS via this same Uplink.
...
The reference architecture utilizes two Kubernetes clusters to break the architecture into dev and prod environments. Inside of those environments are pods that host the router and subgraphs.

Additionally, the cluster contains pods for trace collection and a K6 load testing with Grafana for viewing the results.

In the case of both the dev and prod clusters, the router fetches its schema via the Schema Registry / Uplink. The schema is pushed to the registry for each subgraph using CI/CD via Github Actions.
...
## Components
### Router
The Router is [deployed via Github Actions](#deploy-router) to Kubernetes by utilizing the Helm charts provided for router deployments.
...
The router configuration is provided in the `values.yaml` file in `router.router.configuration`. These config values are provided to the router at runtime.
...
### Subgraphs
Each subgraph is [deployed via Github Actions](#development-mergepushdeploy) (including publishing their schema to the Schema Registry) to Kubernetes utilizing a Helm chart. Each subgraph is deployed to their own Kubernetes namespace and the subgraphs utilize a `HorizontalPodAutoscaler`
...
The OpenTelemetry Collector is [deployed via Github Actions](#deploy-open-telemetry-collector) to Kubernetes utilizing the `otel/opentelemetry-collector-contrib` docker image.
...
### K6 and Grafana
The [K6 Operator](https://github.com/grafana/k6-operator) is [deployed via Github Actions](#deploy-open-telemetry-collector) to Kubernetes. The test results are exported to InfluxDB to be viewed in Grafana.
...
This architecture utilizes Github Actions for its CI/CD. Included in this is:
