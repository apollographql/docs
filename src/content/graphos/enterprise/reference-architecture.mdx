...
The README walks you through everything from setup to CI/CD to load testing the resulting architecture. You can utilize this page as a reference for how the architecture
is setup.
...
The development environment consists of a the router and its subgraphs hosted in a Kubernetes cluster in either AWS or GCP. The subgraph schemas
are pushed to Apollo GraphOS via the Schema Uplink and verified using schema checks. The resulting supergraph schema is made available to the Router. The Router reports usage metrics back to GraphOS.
...
- The router and subgraphs send their OpenTelemetry data to a collector to be viewable in Zepkin
- The Load Testing infrastructure (K6 Load Tester, InfluxDB, Grafana) are deployed. K6 is configured to send traffic to the router.
...
This section will summarize the various services and runtimess that make up the architecture. This includes the router, subgraphs, OpenTelemetry, Zipkin, K6, and Grafana.
