...
Your router is provisioned automatically whenever you create a cloud supergraph in GraphOS Studio, or whenever you create a new [variant](../graphs/#variants) for an existing cloud supergraph. Each variant has its own distinct router.
...
Cloud routing on the Dedicated plan has a wider [variety of options](./dedicated#runs-on-aws). Region selection for cloud routers is only available on the Dedicated plan. <TrackableLink href="https://www.apollographql.com/contact-sales?type=dedicated&referrer=docs" eventName="content_contact_cloud">Contact Sales</TrackableLink> to learn more. 
...
Opting out of router updates to cloud routers isn't currently supported. Advanced version management will be available in [Cloud Dedicated](./dedicated) in 2024.
...
You can upgrade to [Cloud Dedicated](./dedicated) for control over scaling and performance. <TrackableLink href="https://www.apollographql.com/contact-sales?type=dedicated&referrer=docs" eventName="content_contact_cloud">Contact Sales</TrackableLink> for a free trial.
...
They're only available inside the runtime environment where you have total control over when those secrets are resolved in configuration.
...
GraphOS Cloud on AWS is a managed API solution. It runs the Apollo Router on AWS infrastructure to provide a high-performance, configurable GraphQL router. All [Dedicated](./dedicated) cloud routers run on AWS.
