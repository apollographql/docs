...
Cloud routing for Serverless plans is only available in Chicago, USA. Apollo's infrastructure has interconnects with major public cloud providers, so latency is minimal. For example, the latency between Chicago and AWS in Virginia is approximately 10 ms.
...
You can see your cloud router's current status in the GraphOS Studio from your supergraph's overview page on the right side under **Cloud Router Details**.
