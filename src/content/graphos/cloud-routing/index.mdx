...
Opting out of router updates to cloud routers is currently not possible. Advanced version management will be available in [Cloud Dedicated](./cloud-dedicated) in 2024.
...
You may receive out of memory errors when using cloud routers on the [Serverless plan](./#router-comparison). Since the Serverless plan offers limited underlying compute resources, Serverless cloud routing offers a fixed amount of throughput capacity.
...
You can upgrade to [Cloud Dedicated](./cloud-dedicated) for control over scaling and performance. <TrackableLink href="mailto:cloud@apollographql.com" eventName="content_contact_cloud">Get in touch</TrackableLink> for a free trial.
