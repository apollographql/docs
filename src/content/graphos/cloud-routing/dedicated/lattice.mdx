...
This guide shows how to create Lattice services and share them with GraphOS via the Apollo AWS organization that hosts cloud routers.
...
When you add a private subgraph to a cloud supergraph, GraphOS checks that the subgraph's domain matches one of the Lattice services associated with your GraphOS organization.

As a second line of defense, supergraphs use AWS IAM permissions and [SigV4](https://docs.aws.amazon.com/AmazonS3/latest/API/sig-v4-authenticating-requests.html) to allow only traffic to subgraphs in the same GraphOS organization.
...
- You can use Amazon VPC Lattice in your organization for other uses than subgraph hosting. GraphOS associates your Lattice services with its own service network. You can associate a Lattice service with multiple service networks.
...
A VPC Lattice _target group_ is a collection of targets, or compute resources, that run your application or service. In this setup, you use a target group to encapsulate all subgraphs you want to route traffic to.
...
In this step, you create a single VPC Lattice service with listener rules for each of your targets.
...
The next steps differ depending on if you're creating a [new graph](#setup-for-new-graphs) in GraphOS or adding this [service to an existing graph](#setup-for-existing-graphs).
...
Refer to the [Lattice management page](./lattice-management) for more information on this and how to further restrict access to subgraphs and remove resource shares.
