...
If you haven't configured Amazon VPC Lattice yet or need to add new subgraphs, follow the [configuration guide](./lattice).
...
You can also use the **Metrics** tab to get a quick overview if traffic is flowing to your private subgraphs.
...
To remove GraphOS access to private subgraphs, you need to remove both resource shares and service network associations. Keep in mind that any existing supergraph that sends traffic to your private subgraphs will stop working once you remove access.
...
3. Select the resource share(s) associated with the Apollo AWS organization and click the **Delete** button in the top right corner.
...
Deleting the network association can take a few seconds. Once the network association is deleted, GraphOS cannot contact your subgraphs anymore.
