...
- _19,000 / 60 = 317_, so the graph has a peak of **317 RPS**.
- _317 / 25 = 13_, so this workload requires at least **13 GCUs** to operate.
...
Cloud Dedicated supports mission-critical production workloads, and you can scale performance to meet the needs of your GraphQL APIs. Dedicated supports up to 10 GCUs per GraphOS variant.

Dedicated starts with 1 GCU per variant. You can modify the number GCUs from either:
* [Studio](#edit-gcus-in-studio)
* [Platform API](#edit-gcus-via-apis)

<Note>
Modifying the number of GCUs takes about 1 minute to provision.
</Note>

### Edit GCUs in Studio

You can add GCUs from the **Cloud Router** page in Studio. Click **Manage Capacity** to the right of **Router Capacity**.
...
You can modify the number of GCUs per a Dedicated variant with the following mutation from the [GraphOS Platform API](/graphos/platform-api/).
