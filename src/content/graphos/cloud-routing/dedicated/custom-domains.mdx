...
The _custom domains_ feature lets you replace the default `apollographos.net` [router endpoint URL](./quickstart#router-endpoint-url) with your own subdomain, for example `api.apollographql.com`.
...
2. Open a support ticke with Apollo support
...
Your request may take up to 2 business days to process. Once Apollo has processed your custom domain request, your new CNAME will be live.

### Zero downtime migrations
Custom domains supports zero downtime migrations where you want to move `api.mycompany.com` from a different host to GraphOS Cloud. To do this, create a support ticket first and we'll guide you through the process of getting ready for a zero downtime migration using an existing CNAME.
