...
subtitle: Learn what to consider when migrating from a self-hosted graph to a cloud graph
...
Dedicated cloud routers currently support all [premium router features](/router/enterprise-features) except for [safelisting with persisted queries](/router/configuration/persisted-queries/) and [automatic persisted queries](/apollo-server/performance/apq/). Support for these is on the roadmap.
