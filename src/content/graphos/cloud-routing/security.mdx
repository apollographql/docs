---
title: GraphOS Cloud on AWS security overview
subtitle: ""
description: ""
---
...
This article provides a security overview of GraphOS Cloud on AWS. 
It's intended for existing and potential Apollo users who run their federated GraphQL API on Apollo GraphOS Cloud on AWS.

<Note>

All [Dedicated](./dedicated) cloud routers run on AWS. Serverless cloud routers run on other providers but may be transitioned to AWS starting MMMM, DD, YYYY.
...
Apollo users are responsible for making their own independent assessment of the information in this document.
...
- Providing cloud router configurations and GraphQL schema that meet their security requirements
...
To mitigate the impact of security incidents, cloud routers are deployed in small groups in isolated cells with no shared network infrastructure.
...
Apollo regularly contracts third-party firms to perform security audits of the Apollo Router. Apollo also performs automated security scans and collaborates with security researchers through its responsible disclosure policy, available on the [Apollo Router GitHub repository](https://github.com/apollographql/router).
...
Cloud router secrets are encrypted at rest using [AWS Key Management Service (KMS)](https://aws.amazon.com/kms/). Cloud router instances use AWS IAM Roles to access secrets and can only access secrets related to their respective cloud routers.

Apollo employees can't access cloud router secrets or their decryption keys.

### Security updates

If a security issue is found that impacts cloud routers, Apollo creates an updated cloud router build. Cloud router builds are comprised of the underlying Apollo Router version and a unique number identifying the build number, for example, `1.30.1-4567`.
...
During the update process, Apollo creates new cloud router instances and progressively switches traffic to them. If any issue arises during this deployment, Apollo automatically rolls back traffic to the previous cloud router instances.

## GraphOS Cloud compliance

Apollo GraphOS Cloud on AWS meets the standards for SOC2 Type II.
Apollo regularly contracts with third-party firms to perform security audits and penetration tests of the Apollo GraphOS Cloud on AWS infrastructure.