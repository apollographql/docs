...
- You have graphs created before May 21, 2024 AND
- Any of your graphs' cloud routers run on Fly.io

You can check if any of your graph's use cloud routers running on Fly.io in [GraphOS Studio](https://studio.apollographql.com?referrer=docs-content. Go to a graph variant's **Overview** page and look for the migration alert.
...
On AWS, Serverless cloud routers will be able to process a consistent 10 requests per second (RPS) of throughput.
...
On Fly.io, cloud routers were hosted in Chicago (ord). On AWS, they will be hosted in Northern Virginia (us-east-1). This means faster performance, particularly for customers running subgraph servers on AWS, especially if they are in or near us-east-1.
...
To get started, create a new graph or variant in GraphOS Studio.
All Serverless cloud routers created on or after May 21, 2024 automatically run on AWS.

[To-do: Screenshot showing AWS router]

You need at least one subgraph during this process. See the [getting started](/graphos/quickstart/cloud/#from-an-existing-account) for details on creating a new graph.
...
Simulate production traffic before going live. Serverless on AWS offers less throughput than on Fly.io. If you exceed throughput limits, your clients will receive 429 errors.
