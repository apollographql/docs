...
description: Learn how to report GraphQL operation and field usage metrics from the Apollo GraphOS Router, Apollo Server, and third-party server libraries.
---
...
How you report operation metrics to GraphOS depends on whether you're [using the GraphOS Router or Apollo Server](#from-graphos-router-or-apollo-server) or whether you're [using a third-party server](#from-a-third-party-server-advanced).
...
1. Obtain a _graph API key_ from GraphOS Studio.
   ...
   The graph API key you use must have at least the [**Contributor** role](../org/members#organization-wide-member-roles) to report metrics for a non-protected variant, and either an **Org Admin** or **Graph Admin** role to report metrics for a [protected variant](../graphs/management/#protected-variants-enterprise-only).
...
- You must first configure your server to [send operation metrics to GraphOS](#from-graphos-router-or-apollo-server).
- To report requests:
...
Calculating execution metrics can affect performance for large queries or high-traffic graphs. This is especially true for [federated graphs](/federation) because a subgraph includes each operation's full trace data in its response.
