...
description: Learn how to report operation and field usage metrics to GraphOS
---

GraphOS Studio offers visualizations of metrics like request rate, latency, and more to help you analyze your supergraph's performance. Studio also lets you analyze how individual fields in your GraphQL requests are being used by clients. To analyze operation metrics and field usage in Studio, you first need to report them to GraphOS.

<EnterpriseFeature
  text={`Reporting metrics from [Apollo Server](/apollo-server) or a [monograph](../graphs/#monographs) requires an [Enterprise or legacy Team plan](../orgs/plans). Connecting a self-hosted router to GraphOS requires an [Enterprise plan](https://www.apollographql.com/pricing/). If your organization doesn't currently have an Enterprise plan, you can test out this functionality by signing up for a free [Enterprise trial](https://studio.apollographql.com/signup?type=enterprise-trial&referrer=docs-content).
`}
/>
...
2. Obtain the **graph ref** for the graph and variant you want to report metrics for.
   ...
   Your variant's graph ref is shown at the top of its README page in GraphOS Studio. It has the format `graph-id@variant-name` (such as `my-graph@staging`).
...
The GraphOS reporting endpoint accepts batches of traces and metrics that are encoded in **protocol buffer** format. Each trace corresponds to the execution of a single GraphQL operation, including a breakdown of the timing and error information for each field that's resolved as part of the operation.
...
This document describes how to create a report whose `tracesPerQuery` objects consist solely of a list of detailed execution traces in the `trace` array. GraphOS now allows your server to describe usage as a mix of detailed execution traces and pre-aggregated metrics (released in Apollo Server 2.24), which leads to much more efficient reports. This document does not yet describe how to generate these metrics. It also does not describe how to support the "referencing operations" feature of [the Insights page](./field-usage/). We strongly encourage developers to contact Apollo support at **support@apollographql.com** to discuss their use case prior to building their own reporting agent using this module.
...
**The reporting endpoint rejects reports that are older than 50 minutes.** If you see an error like `Rejecting report from service {your service} with skewed timestamp`, make sure your traces are current and that your timestamp calculations are accurate.
...
Your GraphQL router or server can report one or both of following field usage metrics:

- **Requests**: How many times an operation that requests a particular field has been observed
- **Field executions**: How many times the resolver for a particular field has been executed
...
These requests _do_ still contribute to most features of Studio, such as schema checks, the Insights page, and the "Request" metrics on the Insights page.
...
Make sure to pass a _number_ (like `0.01`), _not_ a function that always _returns_ the same number (like `() => 0.01`), which has a [different effect](#custom-sampling).

</Caution>

In this case, whenever field-level instrumentation _is_ enabled for a particular request, Apollo Server reports it to Studio with a weight based on the given probability. The "executions" statistic on the Insights page (along with execution timing hints) is scaled by this weight.
