...
If you have a [self-hosted supergraph](../quickstart/self-hosted), you need to [connect your router to GraphOS](./sending-operation-metrics) to report metrics. The reporting mechanism is the same if you're using [Apollo Server](/apollo-server/) without the Apollo Router. If you're using a third-party server, you need to [set up a reporting agent](./sending-operation-metrics#from-a-third-party-server-advanced).
...
- An operations overview where you can adjust the overviews timeframe and filter it to specific clients
