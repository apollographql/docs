...
Directly below the [**Operations Overview**](#operations-overview), you can find the following overall operation metric visualizations:
...
The operations included in the **request** metric visualizations are queries, mutations, and subscription registrations. Request visualizations don't include data on **subscription notifications**, which are shown separately in the subscription notification charts.
...
If you view a specific subscription's detail page, you see visualizations containing data pertaining only to that subscription. These include all the [subscription notification visualizations](#operation-metrics) shown in the operations overview. However, a subscription's detail page shows subscription registration metrics instead of the request visualizations on the operations overview.
...
The **Traces** tab for an operation provides a breakdown of timing and error information for _each field_ resolved as part of that operation. Traces help you identify opportunities to improve your supergraph's overall performance by optimizing whichever resolvers currently act as a bottleneck.
