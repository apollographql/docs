...
The subscription notification rate is the total number of updates ("notifications") sent to clients as the result of all [subscriptions](/apollo-server/data/subscriptions/) over a specified time period. A sudden spike in subscription notification rate could indicate a surge in real-time activity and help you anticipate the need for scaling your infrastructure.
...
- **Step 2**. When a user joins a chat or opens a messaging window, the client sends a subscription request to the graph. For example:
