...
If you view a specific subscription's detail page, you see charts containing data pertaining only to that subscription. These include all the [subscription notification charts](#operation-metrics) shown in the operations overview. However, a subscription's detail page shows subscription registration metrics instead of the request charts on the operations overview.
