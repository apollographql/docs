...
- The main insights view, which displays an [**Operations Overview**](#operations-overview) and overall [request metrics](#request-metrics) by default. When you click an operation name from the left sidebar or the overview, this view refines to show [detailed information](#operation-details) about the individual operation. You can always return to the overall view by clicking **← Back to All Operations** in the upper right.
...
The **Operations Overview** is top and center on the **Insights** page because it summarizes your supergraph's recent usage and performance. It includes:

- The **rate of requests** sent to your router and the operations with the highest request rates
- Your supergraph's **p95 service time** and the operations with the slowest p95 service time
- Your supergraph's **error percentage** and the operations with the highest error percentage
...
The overview displays overall values for _all_ operations, along with values broken down by operation name. You can click an operation name to view more [detailed information](#operation-details) for it. You can also filter the overview by timeframe and client in the top right corner.
...
More overall [request metrics](#request-metrics) are directly below the operations overview.
...
As with the operations overview, you can click an operation to dive into its details.
...
When you click an operation name, you open detailed information for that operation. This information includes the following:
...
To the right of the operation signature and query plan, you can view the operations **usage**, **error**, or **traces** by toggling the labels above the operation signature or query plan.
...
- Usage metrics include requests per client and other [request metrics](#request-metrics).
- Error metrics show the error count over time.
- Trace metrics display [resolver-level traces](#resolver-level-traces) for an individual operation.
