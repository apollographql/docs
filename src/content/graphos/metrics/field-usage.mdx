...
All plans have access to field usage metrics for standard object fields. For example, given a `User` type like this:
...
For example, given a `UserInput` input type like this:
...
Only enterprise organizations can view usage metrics on `UserInput.name` or `InventoryStatus.IN_STOCK`.
...
You can filter field types by using the dropdown in the left sidebar on the **Insights** page:
