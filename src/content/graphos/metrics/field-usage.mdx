...
Organizations with an Enterprise plan can view insights on input object fields and enum values in addition to standard object fields. To access these metrics you must use the Apollo Router to [report metrics](./sending-operation-metrics) and configure [extended reference reporting](/router/configuration/overview/#extended-reference-reporting) in your router.
