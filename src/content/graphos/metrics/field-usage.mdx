...
> **Note**: Reporting field metrics requires additional steps beyond operation metrics. See [Reporting field usage](./sending-operation-metrics#reporting-field-usage) for details.
...
This page focuses on field metrics accessible from the left sidebar. Refer to [Operation metrics in GraphOS](./operations) for more information on the other metrics on this page, including [resolver-level traces](/operations#resolver-level-traces).
...
As you can see, the **requests** and **executions** for a single field can differ significantly! There are many possible reasons for this, described below.
