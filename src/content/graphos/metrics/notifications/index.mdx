...
- [Performance alerts](./performance-alerts) whenever a metric such as error percentage or request latency exceeds a particular threshold
...
- [Build status notifications](./build-status-notification) whenever Apollo attempts to compose a supergraph schema for your federated graph

Refer to the notification type page for setup instructions.
...
- Schema proposals, and therefore notifications about them, are only available with an Enterprise plan.
