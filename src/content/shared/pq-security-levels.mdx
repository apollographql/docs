...
Before applying this configuration, ensure your PQL contains all GraphQL operations that _all active versions_ of your clients execute. If you enable safelisting without ensuring this, your router will reject any unpublished client operations.
...
With `log_unknown` set to true, the router [logs](/router/logging) all rejected operations, including those registered to your PQL but that used the full operation string rather than the PQL-specified ID.
