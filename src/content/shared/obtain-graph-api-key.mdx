...
>
> API keys are not available for [development graphs](/docs/studio/dev-graphs/).
...
3. Open your graph's Settings page and select the API Keys tab. Click **Create New Key**. Give your key a name, such as `Production`. This helps you keep track of each API key's use.
   ...
   > If you don't see the API Keys tab, you don't have sufficient permissions for your graph. Only organization members with the `Org Admin` or `Graph Admin` role can manage graph API keys. [Learn more about member roles.](/studio/org/members/)
