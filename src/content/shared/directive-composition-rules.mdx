...
<RuleExpansionPanel
  ...
  whatItDoes="Checks if a non-[`repeatable`](https://spec.graphql.org/draft/#sel-GAHnBRVABeBoB_2T) directive has been applied to a schema element in different subgraphs with different arguments. In this case, arguments values are merged using [directive configured strategies](/apollo-server/schema/directives/#in-subgraphs)."
...
<RuleExpansionPanel
  ...
  rationale="The [`@override`](/federation/federated-types/federated-directives/#override) directive indicates that an object field is now resolved by this subgraph instead of other subgraph where it's also defined. The directive can't work unless you specify the subgraph to resolve the field from."
>
