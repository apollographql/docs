...
type Product {
  id: ID!
  name: String
  price(currency: String): Float #highlight-line
}
```
...
input OrderInput {
  product: ProductInput #highlight-line
}
```
...
Indicates that an executable directive definition is marked repeatable in only some of the subgraphs and will _not_ be repeatable in the supergraph.
